...
extern "C" {
   ...
   */
	struct ndpi_detection_module_struct
	    *ndpi_init_detection_module(u_int32_t ticks_per_second,
					void *(*__ndpi_malloc) (unsigned long
								size),
   ...
   */
	u_int8_t ndpi_detection_flow_protocol_history_contains_protocol(struct
									...
									*flow,
									u_int16_t
									protocol_id);
	unsigned int ndpi_find_port_based_protocol(struct
						   ndpi_detection_module_struct
						   *ndpi_struct, u_int8_t proto,
						   ...
						   u_int32_t dhost,
						   u_int16_t dport);
	unsigned int ndpi_guess_undetected_protocol(struct
						    ndpi_detection_module_struct
						    *ndpi_struct,
						    u_int8_t proto,
						    u_int32_t shost,
	...
	int matchStringProtocol(struct ndpi_detection_module_struct
				*ndpi_struct, struct ndpi_flow_struct *flow,
				char *string_to_match,
				u_int string_to_match_len);

	int ndpi_load_protocols_file(struct ndpi_detection_module_struct
				     *ndpi_mod, char *path);
	u_int ndpi_get_num_supported_protocols(struct
					       ndpi_detection_module_struct
					       *ndpi_mod);
