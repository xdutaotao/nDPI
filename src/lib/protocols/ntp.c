...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_NTP,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (!(packet->udp->dest == htons(123)
	      || packet->udp->source == htons(123)))
		goto exclude_ntp;

	NDPI_LOG(NDPI_PROTOCOL_NTP, ndpi_struct, NDPI_LOG_DEBUG,
		 "NTP port detected\n");
