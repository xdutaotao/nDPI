...
static u_int getNameLength(u_int i, const u_int8_t * payload, u_int payloadLen)
{
	if (payload[i] == 0x00)
		...
		if (off == 0)	/* Bad packet */
			...
			return (off +
				getNameLength(i + off, payload, payloadLen));
	}
}
...
struct dns_packet_header {
	u_int16_t transaction_id, flags, num_queries, answer_rrs, authority_rrs,
	    additional_rrs;
...
#define NDPI_MAX_DNS_REQUESTS			16
	...
	NDPI_LOG(NDPI_PROTOCOL_DNS, ndpi_struct, NDPI_LOG_DEBUG,
		 "search DNS.\n");
	...
	if (((dport == 53) || (sport == 53) || (dport == 5355))
		...
		struct dns_packet_header header, *dns =
		    (struct dns_packet_header *)&packet->payload[i];
		u_int8_t is_query, ret_code, is_dns = 0;
		u_int32_t a_record[NDPI_MAX_DNS_REQUESTS] =
		    { 0 }, query_offset, num_a_records = 0;
		...
		if (is_query) {
			...
			if ((header.num_queries <= NDPI_MAX_DNS_REQUESTS)	/* Don't assume that num_queries must be zero */
			    &&(((header.answer_rrs > 0)
				&& (header.answer_rrs <= NDPI_MAX_DNS_REQUESTS))
			       || ((header.authority_rrs > 0)
				...
				if (header.answer_rrs > 0) {
					...
					for (num = 0; num < header.answer_rrs;
					     num++) {
						u_int16_t data_len;
		...
		if (is_dns) {
			...
			flow->protos.dns.num_queries =
			    (u_int8_t) header.num_queries,
			    flow->protos.dns.num_answers =
			    (u_int8_t) (header.answer_rrs +
					header.authority_rrs +
					header.additional_rrs),
			...
			memcpy(&flow->protos.dns.query_type,
			       &packet->payload[i], 2);
			flow->protos.dns.query_type =
			    ntohs(flow->protos.dns.query_type), i += 2;

			memcpy(&flow->protos.dns.query_class,
			       &packet->payload[i], 2);
			flow->protos.dns.query_class =
			    ntohs(flow->protos.dns.query_class), i += 2;
			...
			if (packet->detected_protocol_stack[0] ==
				 ...
				 */
				NDPI_LOG(NDPI_PROTOCOL_DNS, ndpi_struct,
					 NDPI_LOG_DEBUG, "found DNS.\n");
