...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_CONTENT_MMS,
				NDPI_REAL_PROTOCOL);
}
			 ...
			 struct ndpi_flow_struct *flow)
{
	...
	if (packet->payload_packet_len >= 20) {
		if (flow->l4.tcp.mms_stage == 0 && packet->payload[4] == 0xce
		    ...
		    && packet->payload[13] == 0x4d
		    && packet->payload[14] == 0x53
		    && packet->payload[15] == 0x20) {
			NDPI_LOG(NDPI_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG,
				 "MMS: MSMMS Request found \n");
		...
		if (flow->l4.tcp.mms_stage == 2 - packet->packet_direction
		    ...
		    && packet->payload[12] == 0x4d
		    && packet->payload[13] == 0x4d
		    && packet->payload[14] == 0x53
		    && packet->payload[15] == 0x20) {
			NDPI_LOG(NDPI_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG,
				 "MMS: MSMMS Response found \n");
	...
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
		...
		NDPI_LOG(NDPI_CONTENT_MMS, ndpi_struct, NDPI_LOG_DEBUG,
			 "MMS avoid early exclude from http\n");
	}
#endif /* NDPI_PROTOCOL_HTTP */
