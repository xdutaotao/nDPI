...
static void ndpi_check_citrix(struct ndpi_detection_module_struct *ndpi_struct,
			      struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		...
		if ((flow->l4.tcp.citrix_packet_id == 3)
			...
			if (payload_len == 6) {
				char citrix_header[] =
				    { 0x07, 0x07, 0x49, 0x43, 0x41, 0x00 };

				if (memcmp
				    (packet->payload, citrix_header,
				     sizeof(citrix_header)) == 0) {
					NDPI_LOG(NDPI_PROTOCOL_CITRIX,
			...
			struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_CITRIX, ndpi_struct, NDPI_LOG_DEBUG,
		 "citrix detection...\n");
