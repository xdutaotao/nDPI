...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_RTSP,
				protocol_type);
}
			      ...
			      *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_RTSP, ndpi_struct, NDPI_LOG_TRACE,
		 "RTSP detection...\n");
	...
	if (flow->packet_counter < 3
		...
		NDPI_LOG(NDPI_PROTOCOL_RTSP, ndpi_struct, NDPI_LOG_DEBUG,
			 "maybe handshake 2; need next packet.\n");
		return;
	}

	if (packet->payload_packet_len > 20
	    && flow->rtsprdt_stage == 2 - packet->packet_direction) {
		char buf[32];
		...
		if ((memcmp(packet->payload, "RTSP/1.0 ", 9) == 0)
			...
			NDPI_LOG(NDPI_PROTOCOL_RTSP, ndpi_struct,
				 NDPI_LOG_TRACE, "found RTSP/1.0 .\n");
			if (dst != NULL) {
				NDPI_LOG(NDPI_PROTOCOL_RTSP, ndpi_struct,
					 NDPI_LOG_TRACE, "found dst.\n");
				ndpi_packet_src_ip_get(packet,
						       &dst->rtsp_ip_address);
			...
			if (src != NULL) {
				NDPI_LOG(NDPI_PROTOCOL_RTSP, ndpi_struct,
					 NDPI_LOG_TRACE, "found src.\n");
				ndpi_packet_dst_ip_get(packet,
						       &src->rtsp_ip_address);
	...
	if (packet->udp != NULL
	    ...
	    ((NDPI_COMPARE_PROTOCOL_TO_BITMASK
	      (flow->excluded_protocol_bitmask, NDPI_PROTOCOL_RTP) == 0)
#endif
#ifdef NDPI_PROTOCOL_RTCP
	     ||
	     (NDPI_COMPARE_PROTOCOL_TO_BITMASK
	      (flow->excluded_protocol_bitmask, NDPI_PROTOCOL_RTCP) == 0)
#endif
	    )) {
		NDPI_LOG(NDPI_PROTOCOL_RTSP, ndpi_struct, NDPI_LOG_DEBUG,
