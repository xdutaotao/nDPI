...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_KERBEROS,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len >= 4
		...
		if (packet->payload_packet_len > 19
			...
			|| packet->payload[19] == 0x0c
			|| packet->payload[19] == 0x0d
			|| packet->payload[19] == 0x0e)) {
			NDPI_LOG(NDPI_PROTOCOL_KERBEROS, ndpi_struct,
				 NDPI_LOG_DEBUG, "found KERBEROS\n");
		...
		if (packet->payload_packet_len > 21 &&
		    ...
		    (packet->payload[21] == 0x0a ||
		     packet->payload[21] == 0x0c || packet->payload[21] == 0x0d
		     || packet->payload[21] == 0x0e)) {
			NDPI_LOG(NDPI_PROTOCOL_KERBEROS, ndpi_struct,
				 NDPI_LOG_DEBUG, "found KERBEROS\n");
			ndpi_int_kerberos_add_connection(ndpi_struct, flow);
			return;
	...
	NDPI_LOG(NDPI_PROTOCOL_KERBEROS, ndpi_struct, NDPI_LOG_DEBUG,
		 "no KERBEROS detected.\n");
