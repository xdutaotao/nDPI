...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_RTCP,
		      ...
		      struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_RTCP, ndpi_struct, NDPI_LOG_DEBUG,
		 "search for RTCP.\n");
	...
	if (packet->tcp != NULL) {
		...
		if (packet->payload_packet_len > 13
			...
			NDPI_LOG(NDPI_PROTOCOL_RTCP, ndpi_struct,
				 NDPI_LOG_DEBUG, "found rtcp.\n");
		...
		NDPI_LOG(NDPI_PROTOCOL_RTCP, ndpi_struct, NDPI_LOG_DEBUG,
			 "calculating dport over udp.\n");
		if (((packet->payload_packet_len >= 28
		      || packet->payload_packet_len <= 1200)
		     && ((packet->payload[0] == 0x80)
			 && ((packet->payload[1] == 0xc8)
			     || (packet->payload[1] == 0xc9))
