...
static void ndpi_check_netflow(struct ndpi_detection_module_struct *ndpi_struct,
			       struct ndpi_flow_struct *flow)
{
	...
	if ((packet->udp != NULL) && (payload_len >= 24)) {
		...
		if (((version == 1) && (when == 0))
			...
			NDPI_LOG(NDPI_PROTOCOL_NETFLOW, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found netflow.\n");
...
void ndpi_search_netflow(struct ndpi_detection_module_struct *ndpi_struct,
			 struct ndpi_flow_struct *flow)
{
	NDPI_LOG(NDPI_PROTOCOL_NETFLOW, ndpi_struct, NDPI_LOG_DEBUG,
		 "netflow detection...\n");
