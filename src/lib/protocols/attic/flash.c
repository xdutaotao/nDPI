...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_FLASH,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->l4.tcp.flash_stage == 0 && packet->payload_packet_len > 0
		...
		if (packet->tcp->psh == 0) {
			NDPI_LOG(NDPI_PROTOCOL_FLASH, ndpi_struct,
				 NDPI_LOG_DEBUG, "FLASH pass 1: \n");
			...
			return;
		} else if (packet->tcp->psh != 0
			   && flow->l4.tcp.flash_bytes == 1537) {
			NDPI_LOG(NDPI_PROTOCOL_FLASH, ndpi_struct,
				 NDPI_LOG_DEBUG,
	...
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
		...
		NDPI_LOG(NDPI_PROTOCOL_FLASH, ndpi_struct, NDPI_LOG_DEBUG,
			 "FLASH avoid early exclude from http\n");
	}
#endif /* NDPI_PROTOCOL_HTTP */
