...
static void ndpi_int_secondlife_add_connection(struct
					       ...
					       *ndpi_struct,
					       ...
					       protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_SECONDLIFE,
				protocol_type);
}
			    ...
			    *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /")
			...
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Second Life HTTP 'GET /'' found.\n");
			...
			if (packet->host_line.ptr != NULL
			    ...
			    NDPI_STATICSTRING_LEN(".agni.lindenlab.com:")) {
				u_int8_t x;
				for (x = 2; x < 6; x++) {
					if (packet->
					    host_line.ptr[packet->host_line.
							  len - (1 + x)] ==
					    ':') {
						if ((1 + x +
						     NDPI_STATICSTRING_LEN
	...
	if (packet->udp != NULL) {
		if (packet->payload_packet_len == 46
		    && memcmp(packet->payload,
			      "\x40\x00\x00\x00\x01\x00\xff\xff\x00\x03",
			      10) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Second Life 0xffff0003 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow,
							   NDPI_REAL_PROTOCOL);
			return;
		}
		if (packet->payload_packet_len == 54
		    && memcmp(packet->payload,
			      "\x40\x00\x00\x00\x01\x00\xff\xff\x00\x52",
			      10) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Second Life 0xffff0052 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow,
							   NDPI_REAL_PROTOCOL);
			return;
		}
		if (packet->payload_packet_len == 58
		    && memcmp(packet->payload,
			      "\x40\x00\x00\x00\x01\x00\xff\xff\x00\xa9",
			      10) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Second Life 0xffff00a9 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow,
							   NDPI_REAL_PROTOCOL);
			return;
		}
		if (packet->payload_packet_len > 54
		    && memcmp(packet->payload, "\x40\x00\x00\x00\x01\x00\x08",
			      7) == 0
		    && get_u_int32_t(packet->payload,
				     packet->payload_packet_len - 4) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_SECONDLIFE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Second Life 0x08 detected.\n");
			ndpi_int_secondlife_add_connection(ndpi_struct, flow,
							   NDPI_REAL_PROTOCOL);
			return;
		}
