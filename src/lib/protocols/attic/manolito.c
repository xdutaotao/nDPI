					     ...
					     struct ndpi_flow_struct *flow)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_MANOLITO,
				NDPI_REAL_PROTOCOL);

	if (src != NULL) {
		if (packet->udp != NULL) {
			src->manolito_last_pkt_arrival_time =
			    packet->tick_timestamp;
		}
	}
	if (dst != NULL) {
		if (packet->udp != NULL) {
			dst->manolito_last_pkt_arrival_time =
			    packet->tick_timestamp;
		}
	}
}
...
*/
u_int8_t search_manolito_tcp(struct ndpi_detection_module_struct *ndpi_struct,
			     struct ndpi_flow_struct *flow);
u_int8_t search_manolito_tcp(struct ndpi_detection_module_struct *ndpi_struct,
			     struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_MANOLITO, ndpi_struct, NDPI_LOG_DEBUG,
		 "MANOLITO TCP DETECTION\n");
	...
	} else if ((flow->l4.tcp.manolito_stage == 4 - packet->packet_direction)
	...
	} else if ((flow->l4.tcp.manolito_stage == 6 - packet->packet_direction)
		   && packet->payload_packet_len == 4) {
	...
	if (packet->tcp != NULL) {
		...
		if (flow->detected_protocol_stack[0] == NDPI_PROTOCOL_MANOLITO) {
			...
			if (src != NULL
				...
				NDPI_LOG(NDPI_PROTOCOL_MANOLITO, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "MANOLITO: UDP detected \n");
			...
			if ((packet->payload_packet_len == 20
			     && htons(0x3d4b) == get_u_int16_t(packet->payload,
							       0)
			     && packet->payload[2] == 0xd9
			     && htons(0xedbb) == get_u_int16_t(packet->payload,
							       16))
			    || (packet->payload_packet_len == 25
				&& htons(0x3e4a) ==
				get_u_int16_t(packet->payload, 0)
				&& htons(0x092f) ==
				get_u_int16_t(packet->payload, 20)
				&& packet->payload[22] == 0x20)
			    || (packet->payload_packet_len == 20
				&& !get_u_int16_t(packet->payload, 2)
				&& !get_u_int32_t(packet->payload, 8)
				&& !get_u_int16_t(packet->payload, 18)
				&& get_u_int16_t(packet->payload, 0))
			    ) {	//20B pkt is For PING
				NDPI_LOG(NDPI_PROTOCOL_MANOLITO, ndpi_struct,
					 NDPI_LOG_DEBUG,
