...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_POPO,
				NDPI_REAL_PROTOCOL);
}
			      ...
			      *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		if ((packet->payload_packet_len == 20)
		    ...
		    && get_u_int32_t(packet->payload, 12) == 0
		    && get_u_int32_t(packet->payload, 16) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_POPO, ndpi_struct,
				 NDPI_LOG_DEBUG, "POPO detected\n");
			ndpi_int_popo_add_connection(ndpi_struct, flow);
			return;
		}

		if (NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_POPO)
		    != 0) {
...
#define NDPI_POPO_IP_SUBNET_END ( (220 << 24) + (181 << 16) + (28 << 8) + 238)
			...
			if (ntohl(packet->iph->daddr) >=
			    NDPI_POPO_IP_SUBNET_START
			    && ntohl(packet->iph->daddr) <=
			    NDPI_POPO_IP_SUBNET_END) {
				NDPI_LOG(NDPI_PROTOCOL_POPO, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "POPO ip subnet detected\n");
				ndpi_int_popo_add_connection(ndpi_struct, flow);
				return;
			}
		}
	...
	if (packet->payload_packet_len > 13
	    && packet->payload_packet_len == get_l32(packet->payload, 0)
	    && !get_l16(packet->payload, 12)) {
		register u_int16_t ii;
		for (ii = 14; ii < 50 && ii < packet->payload_packet_len - 8;
		     ++ii) {
			if (packet->payload[ii] == '@')
				if (!memcmp
				    (&packet->payload[ii + 1], "163.com", 7)
				    || (ii <= packet->payload_packet_len - 13
					&& !memcmp(&packet->payload[ii + 1],
						   "popo.163.com", 12))) {
					NDPI_LOG(NDPI_PROTOCOL_POPO,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "POPO  detected.\n");
					ndpi_int_popo_add_connection
					    (ndpi_struct, flow);
					return;
				}
		}
