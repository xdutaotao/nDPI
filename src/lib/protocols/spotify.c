...
static void ndpi_int_spotify_add_connection(struct ndpi_detection_module_struct
					    *ndpi_struct,
...
static void ndpi_check_spotify(struct ndpi_detection_module_struct *ndpi_struct,
			       struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL) {
		...
		if ((packet->udp->source == spotify_port)
			...
			if (payload_len > 2) {
				if (memcmp(packet->payload, "SpotUdp", 7) == 0) {
					NDPI_LOG(NDPI_PROTOCOL_SPOTIFY,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "Found spotify udp dissector.\n");
		...
		if (packet->payload[0] == 0x00 && packet->payload[1] == 0x04 &&
			...
			NDPI_LOG(NDPI_PROTOCOL_SPOTIFY, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Found spotify tcp dissector.\n");
			ndpi_int_add_connection(ndpi_struct, flow,
						NDPI_PROTOCOL_SPOTIFY,
						NDPI_REAL_PROTOCOL);
		}
		...
		if (packet->iph
		    /* IPv4 Only: we need to support packet->iphv6 at some point */
		    ) {
			/* if(packet->detected_protocol_stack[0] == NDPI_PROTOCOL_UNKNOWN) */  {
				...
				if (((ntohl(packet->iph->saddr) & 0xFFFFFC00
				      /* 255.255.252.0 */ ) ==
				     0x4E1F0800 /* 78.31.8.0 */ )
				    || ((ntohl(packet->iph->daddr) & 0xFFFFFC00
					 /* 255.255.252.0 */ ) ==
					0x4E1F0800 /* 78.31.8.0 */ )
				    /* **** */
				    || ((ntohl(packet->iph->saddr) & 0xFFFFFC00
					 /* 255.255.252.0 */ ) ==
					0xC1EBE800 /* 193.235.232.0 */ )
				    || ((ntohl(packet->iph->daddr) & 0xFFFFFC00
					 /* 255.255.252.0 */ ) ==
					0xC1EBE800 /* 193.235.232.0 */ )
				    /* **** */
	...
	NDPI_LOG(NDPI_PROTOCOL_SPOTIFY, ndpi_struct, NDPI_LOG_DEBUG,
		 "exclude spotify.\n");
