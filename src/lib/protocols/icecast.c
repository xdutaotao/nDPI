...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_ICECAST,
			     ...
			     struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_ICECAST, ndpi_struct, NDPI_LOG_DEBUG,
		 "search icecast.\n");

	if ((packet->payload_packet_len < 500 &&
	     packet->payload_packet_len >= 7
	     && memcmp(packet->payload, "SOURCE ", 7) == 0)
		...
		for (i = 0; i < packet->parsed_unix_lines; i++) {
			if (packet->unix_line[i].ptr != NULL
			    && packet->unix_line[i].len > 4
			    && memcmp(packet->unix_line[i].ptr, "ice-",
				      4) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_ICECAST, ndpi_struct,
					 NDPI_LOG_DEBUG, "Icecast detected.\n");
