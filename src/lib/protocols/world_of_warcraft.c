						    ...
						    protocol_type)
{
	ndpi_int_add_connection(ndpi_struct, flow,
				NDPI_PROTOCOL_WORLDOFWARCRAFT, protocol_type);
}
	...
	if (packet->tcp != NULL) {
		if ((packet->payload_packet_len >
			...
			if (packet->user_agent_line.ptr != NULL &&
			    ...
			    NDPI_STATICSTRING_LEN("Blizzard Web Client")
			    && memcmp(packet->user_agent_line.ptr,
				      "Blizzard Web Client",
				      NDPI_STATICSTRING_LEN
				      ("Blizzard Web Client")) == 0) {
				ndpi_int_worldofwarcraft_add_connection
				    (ndpi_struct, flow,
				     NDPI_CORRELATED_PROTOCOL);
				NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT,
		...
		if (packet->payload_packet_len > NDPI_STATICSTRING_LEN("GET /")
			...
			if (packet->user_agent_line.ptr != NULL
			    ...
			    && packet->user_agent_line.len >
			    NDPI_STATICSTRING_LEN("Blizzard Downloader")
			    && packet->host_line.len >
			    NDPI_STATICSTRING_LEN("worldofwarcraft.com")
			    && memcmp(packet->user_agent_line.ptr,
				      "Blizzard Downloader",
				      NDPI_STATICSTRING_LEN
				      ("Blizzard Downloader")) == 0
			    && memcmp(&packet->
		...
		if (packet->payload_packet_len == 50
		    && memcmp(&packet->payload[2],
			      "WORLD OF WARCRAFT CONNECTION",
			      NDPI_STATICSTRING_LEN
			      ("WORLD OF WARCRAFT CONNECTION")) == 0) {
			ndpi_int_worldofwarcraft_add_connection(ndpi_struct,
								flow,
								NDPI_REAL_PROTOCOL);
			NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "World of Warcraft: Login found\n");
			return;
		}
		if (packet->tcp->dest == htons(3724)
		    && packet->payload_packet_len < 70
		    && packet->payload_packet_len > 40
		    && (memcmp(&packet->payload[4], "WoW", 3) == 0
		...
		if (NDPI_SRC_OR_DST_HAS_PROTOCOL
		    (src, dst, NDPI_PROTOCOL_WORLDOFWARCRAFT) != 0) {
			if (packet->tcp->source == htons(3724)
			    && packet->payload_packet_len == 8
			    && get_u_int32_t(packet->payload,
					     0) == htonl(0x0006ec01)) {
				ndpi_int_worldofwarcraft_add_connection
				    (ndpi_struct, flow, NDPI_REAL_PROTOCOL);
				NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "World of Warcraft: connection detected\n");
				return;
			}
		...
		if (flow->l4.tcp.wow_stage == 0) {
			if (ndpi_int_is_wow_port(packet->tcp->source) &&
				...
				if (get_u_int32_t(packet->payload, 2) ==
				    htonl(0xec010100)) {

					NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "probably World of Warcraft, waiting for final packet\n");
					...
					NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "maybe World of Warcraft, need next\n");
		...
		if (flow->l4.tcp.wow_stage == 1) {
			if (packet->payload_packet_len == 325 &&
			    ...
			    (get_u_int16_t
			     (packet->payload,
			      packet->payload_packet_len - 3) == htons(0x2331)
			     || get_u_int16_t(packet->payload,
					      67) == htons(0x2331))
			    &&
			    (memcmp
			     (&packet->payload[packet->payload_packet_len - 18],
			      "\x94\xec\xff\xfd\x67\x62\xd4\x67\xfb\xf9\xdd\xbd\xfd\x01\xc0\x8f\xf9\x81",
			      18) == 0
			     ||
			...
			if (packet->payload_packet_len > 32 &&
				...
				if (get_u_int16_t(packet->payload, 4) == 0) {
					...
					flow->l4.tcp.wow_stage = 2;
					return;
				} else if (get_u_int32_t(packet->payload, 2) ==
					   htonl(0x12050000)) {
					NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "probably World of Warcraft, waiting for final packet\n");
		...
		if (flow->l4.tcp.wow_stage == 2) {
			if (packet->payload_packet_len == 4) {
				...
				NDPI_LOG(NDPI_PROTOCOL_WORLDOFWARCRAFT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "waiting for final packet\n");
				return;
			}
		}
		if (flow->l4.tcp.wow_stage == 0
		    && packet->tcp->dest == htons(1119)) {
