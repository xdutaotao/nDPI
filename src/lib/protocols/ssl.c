	...
	if (packet->payload[0] == 0x16 /* Handshake */ ) {
		...
		if (total_len > 4) {
			...
			if (handshake_protocol == 0x02
				...
				for (i = 9; i < packet->payload_packet_len - 3;
				     i++) {
					if (((packet->payload[i] == 0x04)
					     && (packet->payload[i + 1] == 0x03)
					     && (packet->payload[i + 2] ==
						 0x0c))
					    || ((packet->payload[i] == 0x55)
						&& (packet->payload[i + 1] ==
						    0x04)
						&& (packet->payload[i + 2] ==
						    0x03))) {
			...
			} else if (handshake_protocol ==
				   0x01 /* Client Hello */ ) {
				u_int offset, base_offset = 43;
				u_int16_t session_id_len =
				    packet->payload[base_offset];

				if ((session_id_len + base_offset + 2) <=
				    total_len) {
					u_int16_t cypher_len =
					    packet->payload[session_id_len +
							    base_offset + 2] +
					    (packet->payload[session_id_len +
							     base_offset +
							     1] << 8);
					offset =
					    base_offset + session_id_len +
					    cypher_len + 2;
...
int sslDetectProtocolFromCertificate(struct ndpi_detection_module_struct
				     *ndpi_struct,
				     struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	...
	if ((packet->detected_protocol_stack[0] == NDPI_PROTOCOL_UNKNOWN)
		...
		certificate[0] = '\0';
		rc = getSSLcertificate(ndpi_struct, flow, certificate,
				       sizeof(certificate));
		...
		if (rc > 0) {
			...
			printf("***** [SSL] %s\n", certificate);
#endif
			if (ndpi_match_string_subprotocol
			    (ndpi_struct, flow, certificate,
			     strlen(certificate)) != NDPI_PROTOCOL_UNKNOWN)
		...
		if ((packet->ssl_certificate_num_checks >= 2)
			...
			ndpi_int_ssl_add_connection(ndpi_struct, flow,
						    NDPI_PROTOCOL_SSL);
	}
							    ...
							    *flow)
{
	...
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
	    (ndpi_struct->detection_bitmask,
	     NDPI_PROTOCOL_UNENCRYPED_JABBER) != 0)
		goto check_for_ssl_payload;
#endif
#if defined(NDPI_PROTOCOL_OSCAR)
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
	    (ndpi_struct->detection_bitmask, NDPI_PROTOCOL_OSCAR) != 0)
		goto check_for_ssl_payload;
#endif
#if defined(NDPI_PROTOCOL_GADUGADU)
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
	    (ndpi_struct->detection_bitmask, NDPI_PROTOCOL_GADUGADU) != 0)
		goto check_for_ssl_payload;
#endif
...
check_for_ssl_payload:
	...
	for (a = 5; a < end; a++) {
		...
		if (packet->payload[a] == 't') {
			if (memcmp(&packet->payload[a], "talk.google.com", 15)
			    == 0) {
				NDPI_LOG(NDPI_PROTOCOL_UNENCRYPED_JABBER,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "ssl jabber packet match\n");
				if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
				    (ndpi_struct->detection_bitmask,
				     NDPI_PROTOCOL_UNENCRYPED_JABBER) != 0) {
					ndpi_int_ssl_add_connection(ndpi_struct,
								    flow,
		...
		if (packet->payload[a] == 'A' || packet->payload[a] == 'k'
		    || packet->payload[a] == 'c' || packet->payload[a] == 'h') {
			if (((a + 19) < packet->payload_packet_len
			     && memcmp(&packet->payload[a],
				       "America Online Inc.", 19) == 0)
			    //                        || (end - c > 3 memcmp (&packet->payload[c],"AOL", 3) == 0 )
			    //                        || (end - c > 7 && memcmp (&packet->payload[c], "AOL LLC", 7) == 0)
			    || ((a + 15) < packet->payload_packet_len
				&& memcmp(&packet->payload[a],
					  "kdc.uas.aol.com", 15) == 0)
			    || ((a + 14) < packet->payload_packet_len
				&& memcmp(&packet->payload[a], "corehc@aol.net",
					  14) == 0)
			    || ((a + 41) < packet->payload_packet_len
				&& memcmp(&packet->payload[a],
					  "http://crl.aol.com/AOLMSPKI/aolServerCert",
					  41) == 0)
			    || ((a + 28) < packet->payload_packet_len
				&& memcmp(&packet->payload[a],
					  "http://ocsp.web.aol.com/ocsp",
					  28) == 0)
			    || ((a + 32) < packet->payload_packet_len
				&& memcmp(&packet->payload[a],
					  "http://pki-info.aol.com/AOLMSPKI",
					  32) == 0)) {
				NDPI_LOG(NDPI_PROTOCOL_OSCAR, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "OSCAR SERVER SSL DETECTED\n");

				if (flow->dst != NULL
				    && packet->payload_packet_len > 75) {
					memcpy(flow->dst->oscar_ssl_session_id,
					       &packet->payload[44], 32);
					flow->dst->oscar_ssl_session_id[32] =
					    '\0';
					flow->dst->oscar_last_safe_access_time =
		...
		if (packet->payload[a] == 'm' || packet->payload[a] == 's') {
			if ((a + 21) < packet->payload_packet_len &&
			    (memcmp
			     (&packet->payload[a], "my.screenname.aol.com",
			      21) == 0
			     || memcmp(&packet->payload[a],
				       "sns-static.aolcdn.com", 21) == 0)) {
				NDPI_LOG(NDPI_PROTOCOL_OSCAR, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "OSCAR SERVER SSL DETECTED\n");
				ndpi_int_ssl_add_connection(ndpi_struct, flow,
...
no_check_for_ssl_payload:
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_UNKNOWN) {
		...
		if (!packet->ssl_certificate_detected
		    &&
		    (!(flow->l4.tcp.ssl_seen_client_cert
		       && flow->l4.tcp.ssl_seen_server_cert))) {
			/* SSL without certificate (Skype, Ultrasurf?) */
			ndpi_int_ssl_add_connection(ndpi_struct, flow,
						    NDPI_PROTOCOL_SSL_NO_CERT);
		} else
			ndpi_int_ssl_add_connection(ndpi_struct, flow,
						    NDPI_PROTOCOL_SSL);
	}
...
static u_int8_t ndpi_search_sslv3_direction1(struct ndpi_detection_module_struct
					     *ndpi_struct,
					     struct ndpi_flow_struct *flow)
{
	...
	if ((packet->payload_packet_len >= 5)
		...
		if (packet->payload_packet_len == temp
		    || (temp < packet->payload_packet_len
			&& packet->payload_packet_len > 500)) {
			return 1;
		}

		if (packet->payload_packet_len < temp && temp < 5000
			...
			cert_start =
			    ntohs(get_u_int16_t(packet->payload, 7)) + 5 + 4;
			NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct, NDPI_LOG_DEBUG,
				 "suspected start of certificate: %u\n",
				 cert_start);

			if (cert_start < packet->payload_packet_len
			    && packet->payload[cert_start] == 0x0b) {
				NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct,
					 NDPI_LOG_DEBUG,
		...
		if ((packet->payload_packet_len > temp
			...
			cert_start =
			    ntohs(get_u_int16_t(packet->payload, 7)) + 5 + 5 +
			    4;
			NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct, NDPI_LOG_DEBUG,
				 "suspected start of certificate: %u\n",
				 cert_start);
		...
		if (packet->payload_packet_len >= temp + 5
		    && (packet->payload[temp] == 0x14
			|| packet->payload[temp] == 0x16)
		    && packet->payload[temp + 1] == 0x03) {
			u_int32_t temp2 =
			    ntohs(get_u_int16_t(packet->payload, temp + 3)) + 5;
			...
			if (packet->payload_packet_len >= temp + 5 &&
				...
				temp2 =
				    ntohs(get_u_int16_t
					  (packet->payload, temp + 3)) + 5;
				if (temp + temp2 > NDPI_MAX_SSL_REQUEST_SIZE) {
					return 1;
				}
				temp += temp2;
				NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct,
					 NDPI_LOG_DEBUG, "temp = %u.\n", temp);
				if (packet->payload_packet_len == temp) {
					return 1;
				}
				if (packet->payload_packet_len >= temp + 5 &&
			 ...
			 struct ndpi_flow_struct *flow)
{
...

	{
		...
		if ((packet->payload_packet_len > 5)
			...
			ndpi_int_add_connection(ndpi_struct, flow,
						NDPI_SERVICE_WHATSAPP,
						NDPI_REAL_PROTOCOL);
#endif
			return;
		} else {
			/* No whatsapp, let's try SSL */
			if (sslDetectProtocolFromCertificate(ndpi_struct, flow)
			    > 0)
				return;
		}
	}
	...
	if (packet->payload_packet_len > 40 && flow->l4.tcp.ssl_stage == 0) {
		...
		if (packet->payload[0] == 0x16 && packet->payload[1] == 0x03
			...
			|| packet->payload[2] == 0x02)
		    && (packet->payload_packet_len -
			ntohs(get_u_int16_t(packet->payload, 3)) == 5)) {
			// SSLv3 Record
			NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct, NDPI_LOG_DEBUG,
				 "sslv3 len match\n");
			flow->l4.tcp.ssl_stage = 1 + packet->packet_direction;
	...
	if (packet->payload_packet_len > 40
	    && flow->l4.tcp.ssl_stage == 2 - packet->packet_direction) {
		NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct, NDPI_LOG_DEBUG,
			 "second ssl packet\n");
		// SSLv2 Record
		if (packet->payload[2] == 0x01 && packet->payload[3] == 0x03
		...
		if (packet->payload_packet_len > 40
		    && flow->
		    packet_direction_counter[packet->packet_direction] < 5) {
			NDPI_LOG(NDPI_PROTOCOL_SSL, ndpi_struct, NDPI_LOG_DEBUG,
				 "need next packet\n");
			return;
		}
