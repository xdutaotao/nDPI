					     ...
					     struct ndpi_flow_struct *flow)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_SOULSEEK,
				NDPI_REAL_PROTOCOL);
			      ...
			      *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_SOULSEEK) {
		...
		if (src != NULL)
			NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct,
				 ...
				 != 0 ? 1 : 0,
				 (u_int64_t) packet->tick_timestamp,
				 (u_int64_t)
		...
		if (packet->payload_packet_len == 12
		    && get_l32(packet->payload, 4) == 0x02) {
			if (src != NULL) {
				src->soulseek_last_safe_access_time =
				    packet->tick_timestamp;
				if (packet->tcp != NULL
				    && src->soulseek_listen_port == 0) {
					src->soulseek_listen_port =
					    get_l32(packet->payload, 8);
					return;
				}
			}
		}
	...
	if (dst != NULL && dst->soulseek_listen_port != 0
	    && dst->soulseek_listen_port == ntohs(packet->tcp->dest)
	    && ((u_int32_t)
	...
	if (flow->l4.tcp.soulseek_stage == 0) {
		...
		if (packet->payload_packet_len >= 12
		    ...
		    && get_l32(packet->payload, 4) == 1) {
			while (!get_u_int16_t(packet->payload, index + 2)
			       && (index + get_l32(packet->payload, index)) <
			...
			if (index + get_l32(packet->payload, index) ==
			    packet->payload_packet_len - 4
			    && !get_u_int16_t(packet->payload, 10)) {
				...
				if ((index + 4) <= packet->payload_packet_len && !get_u_int16_t(packet->payload, index + 2))	// for passwd len
				{
					...
					if ((index + 4 + 4) <= packet->payload_packet_len && !get_u_int16_t(packet->payload, index + 6))	// to read version,hashlen
					{
						...
						if (index ==
						    get_l32(packet->payload,
							    0)) {
							NDPI_LOG
							    (NDPI_PROTOCOL_SOULSEEK,
							     ndpi_struct,
							     NDPI_LOG_DEBUG,
							     "Soulseek Login Detected\n");
							ndpi_int_soulseek_add_connection
							    (ndpi_struct, flow);
							return;
						}
					}
		...
		if (packet->payload_packet_len > 8
		    ...
		    && get_l32(packet->payload,
			       0) == packet->payload_packet_len - 4) {
			//Server Messages:
			const u_int32_t msgcode = get_l32(packet->payload, 4);

			if (msgcode == 0x7d) {
				flow->l4.tcp.soulseek_stage =
				...
				const u_int32_t soulseek_listen_port =
				    get_l32(packet->payload, 8);
			...
			if (packet->payload_packet_len == 9
				...
				NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct,
					 NDPI_LOG_TRACE,
					 "Soulseek Size 9 Pierce Firewall\n");
				return;
			}
		...
		if (packet->payload_packet_len > 25
		    ...
		    && !get_u_int16_t(packet->payload, 7)
		    && !get_u_int16_t(packet->payload, 2)) {
			const u_int32_t usrlen = get_l32(packet->payload, 5);

			if (usrlen <=
			    packet->payload_packet_len - 4 + 1 + 4 + 4 + 1 +
			    4) {
				const u_int32_t typelen =
				    get_l32(packet->payload,
					    4 + 1 + 4 + usrlen);
				const u_int8_t type =
				    packet->payload[4 + 1 + 4 + usrlen + 4];
				if (typelen == 1
				    && (type == 'F' || type == 'P'
					|| type == 'D')) {
					NDPI_LOG(NDPI_PROTOCOL_SOULSEEK,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "soulseek detected Pattern command(D|P|F).\n");
					ndpi_int_soulseek_add_connection
					    (ndpi_struct, flow);
					return;
				}
		...
		if (packet->payload_packet_len > 8) {
			if ((packet->payload[0] || packet->payload[1])
			    && get_l32(packet->payload, 4) == 9) {
				/* 9 is search result */
				NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "soulseek detected Second Pkt\n");
			...
			if (get_l32(packet->payload, 0) ==
			    packet->payload_packet_len - 4) {
				const u_int32_t msgcode =
				    get_l32(packet->payload, 4);
				if (msgcode == 0x03 && packet->payload_packet_len >= 12)	//Server Message : Get Peer Address
				{
					const u_int32_t usrlen =
					    get_l32(packet->payload, 8);
					if (usrlen <= packet->payload_packet_len
					    && 4 + 4 + 4 + usrlen ==
					    packet->payload_packet_len) {
						NDPI_LOG(NDPI_PROTOCOL_SOULSEEK,
							 ndpi_struct,
							 NDPI_LOG_DEBUG,
							 "Soulseek Request Get Peer Address Detected\n");
						ndpi_int_soulseek_add_connection
						    (ndpi_struct, flow);
						return;
					}
		...
		if (packet->payload_packet_len == 8
		    && get_l32(packet->payload, 4) == 0x00000004) {
			NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct,
				 NDPI_LOG_DEBUG, "soulseek detected\n");
			ndpi_int_soulseek_add_connection(ndpi_struct, flow);
			return;
		}

		if (packet->payload_packet_len == 4
		    && get_u_int16_t(packet->payload, 2) == 0x00
		    && get_u_int16_t(packet->payload, 0) != 0x00) {
			NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct,
				 NDPI_LOG_DEBUG, "soulseek detected\n");
			ndpi_int_soulseek_add_connection(ndpi_struct, flow);
			return;
		...
		if (packet->payload_packet_len > 8) {
			if (packet->payload[4] == 0x03
			    && get_l32(packet->payload, 5) == 0x00000031) {
				NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "soulseek detected Second Pkt with SIGNATURE :: 0x0331000000 \n");
				ndpi_int_soulseek_add_connection(ndpi_struct,
								 flow);
				return;
			}
	...
	if (flow->l4.tcp.soulseek_stage == 3 && packet->payload_packet_len == 8
	    && !get_u_int32_t(packet->payload, 4)) {

		NDPI_LOG(NDPI_PROTOCOL_SOULSEEK, ndpi_struct, NDPI_LOG_DEBUG,
			 "soulseek detected bcz of 8B  pkt\n");
		ndpi_int_soulseek_add_connection(ndpi_struct, flow);
		return;
	}
