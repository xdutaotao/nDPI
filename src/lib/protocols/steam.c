...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_STEAM,
				NDPI_REAL_PROTOCOL);
}

static void ndpi_check_steam_http(struct ndpi_detection_module_struct
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->user_agent_line.ptr != NULL
	    ...
	    && memcmp(packet->user_agent_line.ptr, "Valve/Steam HTTP Client",
		      23) == 0) {
...
static void ndpi_check_steam_udp1(struct ndpi_detection_module_struct
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	u_int32_t payload_len = packet->payload_packet_len;
	...
	if (flow->steam_stage1 == 0) {
		...
		if ((payload_len > 0)
		    && match_first_bytes(packet->payload, "\x31\xff\x30\x2e")) {
			NDPI_LOG(NDPI_PROTOCOL_STEAM, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Possible STEAM request detected, we will look further for the response...\n");
		...
		if ((payload_len > 0)
		    && match_first_bytes(packet->payload, "\xff\xff\xff\xff")) {
			NDPI_LOG(NDPI_PROTOCOL_STEAM, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Possible STEAM request detected, we will look further for the response...\n");
...
static void ndpi_check_steam_udp2(struct ndpi_detection_module_struct
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	u_int32_t payload_len = packet->payload_packet_len;
...
static void ndpi_check_steam_udp3(struct ndpi_detection_module_struct
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	u_int32_t payload_len = packet->payload_packet_len;
	...
	if (flow->steam_stage3 == 0) {
		...
		if ((payload_len == 4) && (packet->payload[0] == 0x39)
		    && (packet->payload[1] == 0x18)
		    && (packet->payload[2] == 0x00)
		...
		/* This is a packet in another direction. Check if we find the proper response. */
		if ((payload_len == 0)
		    || ((payload_len == 8) && (packet->payload[0] == 0x3a)
			&& (packet->payload[1] == 0x18)
			&& (packet->payload[2] == 0x00)
			&& (packet->payload[3] == 0x00))) {
			NDPI_LOG(NDPI_PROTOCOL_STEAM, ndpi_struct,
