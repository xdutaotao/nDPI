...
static void ndpi_int_ciscovpn_add_connection(struct ndpi_detection_module_struct
					     *ndpi_struct,
					     struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_CISCOVPN,
				NDPI_REAL_PROTOCOL);
}
			  ...
			  struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_CISCOVPN, ndpi_struct, NDPI_LOG_DEBUG,
		 "search CISCOVPN.\n");
	...
	if (packet->udp != NULL) {
		...
		NDPI_LOG(NDPI_PROTOCOL_CISCOVPN, ndpi_struct, NDPI_LOG_DEBUG,
			 "calculated CISCOVPN over udp ports.\n");
	}
	...
	if ((tdport == 10000 && tsport == 10000) ||
		...
		ndpi_int_ciscovpn_add_connection(ndpi_struct, flow);
		    ...
		    (packet->payload[0] == 0xfe &&
		     ...
		     packet->payload[2] == 0x7e && packet->payload[3] == 0x2b)
		   )
	    ) {
