...
static void ndpi_int_ftp_control_add_connection(struct
						ndpi_detection_module_struct
						*ndpi_struct,
						struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_FTP_CONTROL,
				NDPI_REAL_PROTOCOL);
}
...
static void ndpi_check_ftp_control(struct ndpi_detection_module_struct
				   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp->dest == htons(25) || packet->tcp->source == htons(25)) {
		NDPI_LOG(NDPI_PROTOCOL_FTP_CONTROL, ndpi_struct, NDPI_LOG_DEBUG,
			 "Exclude FTP_CONTROL.\n");
	...
	if (flow->ftp_control_stage == 0) {
		...
		if ((payload_len > 0)
		    && ndpi_ftp_control_check_request(packet->payload)) {
			NDPI_LOG(NDPI_PROTOCOL_FTP_CONTROL, ndpi_struct,
				 NDPI_LOG_DEBUG,
		...
		if ((payload_len > 0)
		    && ndpi_ftp_control_check_response(packet->payload)) {
			NDPI_LOG(NDPI_PROTOCOL_FTP_CONTROL, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found FTP_CONTROL.\n");
