...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_TELNET,
				NDPI_REAL_PROTOCOL);
}
		    ...
		    struct ndpi_flow_struct *flow)
{
	...
	if (!(packet->payload[0] == 0xff
	      && packet->payload[1] > 0xf9 && packet->payload[1] != 0xff
	...
	while (a < packet->payload_packet_len - 2) {
		...
		if (!(packet->payload[a] != 0xff ||
		      (packet->payload[a] == 0xff
		       && (packet->payload[a + 1] >= 0xf0)
		       && (packet->payload[a + 1] <= 0xfa))
		      || (packet->payload[a] == 0xff
			  && (packet->payload[a + 1] >= 0xfb)
			  && (packet->payload[a + 1] != 0xff)
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_PROTOCOL_TELNET, ndpi_struct, NDPI_LOG_DEBUG,
		 "search telnet.\n");

	if (search_iac(ndpi_struct, flow) == 1) {

		if (flow->l4.tcp.telnet_stage == 2) {
			NDPI_LOG(NDPI_PROTOCOL_TELNET, ndpi_struct,
				 NDPI_LOG_DEBUG, "telnet identified.\n");
	...
	if ((flow->packet_counter < 12 && flow->l4.tcp.telnet_stage > 0)
	    || flow->packet_counter < 6) {
		return;
	} else {
		NDPI_LOG(NDPI_PROTOCOL_TELNET, ndpi_struct, NDPI_LOG_DEBUG,
			 "telnet excluded.\n");
