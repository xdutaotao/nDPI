...
static void ndpi_int_crossfire_add_connection(struct
					      ...
					      protocol_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_CROSSFIRE,
				protocol_type);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	NDPI_LOG(NDPI_PROTOCOL_CROSSFIRE, ndpi_struct, NDPI_LOG_DEBUG,
		 "search crossfire.\n");
	...
	if (packet->udp != 0) {
		if (packet->payload_packet_len == 25
			...
			NDPI_LOG(NDPI_PROTOCOL_CROSSFIRE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Crossfire: found udp packet.\n");
		...
		if (packet->payload_packet_len > 4
			...
			if (packet->parsed_lines == 8
			    && (packet->line[0].ptr != NULL
				&& packet->line[0].len >= 30
				&&
				(memcmp
				 (&packet->payload[5], "notice/login_big",
				  16) == 0
				 || memcmp(&packet->payload[5],
					   "notice/login_small", 18) == 0))
			    &&
			    memcmp(&packet->payload[packet->line[0].len - 19],
				   "/index.asp HTTP/1.", 18) == 0
			    && (packet->host_line.ptr != NULL
				&& packet->host_line.len >= 13
				&&
				(memcmp(packet->host_line.ptr, "crossfire", 9)
				 == 0
				...
				NDPI_LOG(NDPI_PROTOCOL_CROSSFIRE, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "Crossfire: found HTTP request.\n");
				ndpi_int_crossfire_add_connection(ndpi_struct,
								  flow,
