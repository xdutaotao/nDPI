...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_XBOX,
				NDPI_REAL_PROTOCOL);
}
		      ...
		      struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL) {
		...
		NDPI_LOG(NDPI_PROTOCOL_XBOX, ndpi_struct, NDPI_LOG_DEBUG,
			 "search xbox\n");

		if (packet->payload_packet_len > 12 &&
		    get_u_int32_t(packet->payload, 0) == 0
		    && packet->payload[5] == 0x58
		    && memcmp(&packet->payload[7], "\x00\x00\x00", 3) == 0) {

			if ((packet->payload[4] == 0x0c
			     && packet->payload[6] == 0x76)
			    || (packet->payload[4] == 0x02
				&& packet->payload[6] == 0x18)
			    || (packet->payload[4] == 0x0b
				&& packet->payload[6] == 0x80)
			    || (packet->payload[4] == 0x03
				&& packet->payload[6] == 0x40)
			    || (packet->payload[4] == 0x06
				&& packet->payload[6] == 0x4e)) {

				ndpi_int_xbox_add_connection(ndpi_struct, flow);
				NDPI_LOG(NDPI_PROTOCOL_XBOX, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "xbox udp connection detected\n");
				return;
			}
		}
		...
		if (packet->tcp == NULL
			...
			NDPI_LOG(NDPI_PROTOCOL_XBOX, ndpi_struct,
				 NDPI_LOG_DEBUG, "xbox udp excluded.\n");
