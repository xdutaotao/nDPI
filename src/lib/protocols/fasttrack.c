...
static void ndpi_int_fasttrack_add_connection(struct
					      ndpi_detection_module_struct
					      *ndpi_struct,
					      struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_FASTTRACK,
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len > 6
		...
		NDPI_LOG(NDPI_PROTOCOL_FASTTRACK, ndpi_struct, NDPI_LOG_TRACE,
			 "detected 0d0a at the end of the packet.\n");
		...
		if (memcmp(packet->payload, "GIVE ", 5) == 0
			...
			NDPI_LOG(NDPI_PROTOCOL_FASTTRACK, ndpi_struct,
				 NDPI_LOG_TRACE, "FASTTRACK GIVE DETECTED\n");
		...
		if (packet->payload_packet_len > 50
			...
			u_int8_t a = 0;
			NDPI_LOG(NDPI_PROTOCOL_FASTTRACK, ndpi_struct,
				 NDPI_LOG_TRACE, "detected GET /. \n");
			ndpi_parse_packet_line_info(ndpi_struct, flow);
			for (a = 0; a < packet->parsed_lines; a++) {
				if ((packet->line[a].len > 17
				     && memcmp(packet->line[a].ptr,
					       "X-Kazaa-Username: ", 18) == 0)
				    || (packet->line[a].len > 23
					&& memcmp(packet->line[a].ptr,
						  "User-Agent: PeerEnabler/",
						  24) == 0)) {
					NDPI_LOG(NDPI_PROTOCOL_FASTTRACK,
						 ndpi_struct, NDPI_LOG_TRACE,
						 "detected X-Kazaa-Username: || User-Agent: PeerEnabler/\n");
					ndpi_int_fasttrack_add_connection
					    (ndpi_struct, flow);
					return;
				}
			}
