...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_TOR,
		     ...
		     struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_TOR, ndpi_struct, NDPI_LOG_DEBUG,
		 "search for TOR.\n");
	...
	if (packet->tcp != NULL) {
		...
		NDPI_LOG(NDPI_PROTOCOL_TOR, ndpi_struct, NDPI_LOG_DEBUG,
			 "calculating TOR over tcp.\n");

		if ((((dport == 9001) || (sport == 9001))
		     || ((dport == 9030) || (sport == 9030)))
