...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_IMESH,
				protocol_type);
}
			       ...
			       struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL) {
		...
		if (packet->payload_packet_len == 28
		    && (get_u_int32_t(packet->payload, 0)) == htonl(0x02000000)
		    && get_u_int32_t(packet->payload, 24) == 0
		    && (packet->udp->dest == htons(1864)
			|| packet->udp->source == htons(1864))) {
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
				 NDPI_LOG_DEBUG, "iMesh Login detected\n");
		...
		if (packet->payload_packet_len == 36) {
			if (get_u_int32_t(packet->payload, 0) ==
			    htonl(0x02000000) && packet->payload[4] != 0
			    && packet->payload[5] == 0
				...
				==
				packet->payload[packet->payload_packet_len - 5]
				...
				packet->payload[packet->payload_packet_len -
						5] - 1)) {
				NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
					 NDPI_LOG_DEBUG, "iMesh detected\n");
				ndpi_int_imesh_add_connection(ndpi_struct, flow,
							      NDPI_REAL_PROTOCOL);
				return;
			}
			if (get_u_int16_t(packet->payload, 0) == htons(0x0200)
		...
		if (packet->payload_packet_len == 24
		    && get_u_int16_t(packet->payload, 0) == htons(0x0200)
		    && get_u_int16_t(packet->payload, 2) != 0
		    && get_u_int32_t(packet->payload, 4) == htonl(0x03000084)
		    && (packet->payload[packet->payload_packet_len - 1] ==
			packet->payload[packet->payload_packet_len - 5]
			|| packet->payload[packet->payload_packet_len - 1] -
			1 == packet->payload[packet->payload_packet_len - 5]
			|| packet->payload[packet->payload_packet_len - 1] ==
		...
		if (packet->payload_packet_len == 32
		    && get_u_int32_t(packet->payload, 0) == htonl(0x02000000)
		    && get_u_int16_t(packet->payload, 21) == 0
		    && get_u_int16_t(packet->payload, 26) == htons(0x0100)) {
			if (get_u_int32_t(packet->payload, 4) ==
			    htonl(0x00000081)
			    && packet->payload[11] == packet->payload[15]
			    && get_l16(packet->payload,
				       24) == htons(packet->udp->source)) {
	...
	if (packet->tcp != NULL) {
		if (packet->payload_packet_len == 64
		    && get_u_int32_t(packet->payload, 0) == htonl(0x40000000)
		    && get_u_int32_t(packet->payload, 4) == 0
		    && get_u_int32_t(packet->payload, 8) == htonl(0x0000fcff)
		    && get_u_int32_t(packet->payload, 12) == htonl(0x04800100)
		    && get_u_int32_t(packet->payload, 45) == htonl(0xff020000)
		    && get_u_int16_t(packet->payload, 49) == htons(0x001a)) {
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
				 NDPI_LOG_DEBUG, "found imesh.\n");
			ndpi_int_imesh_add_connection(ndpi_struct, flow,
						      NDPI_REAL_PROTOCOL);
			return;
		}
		if (packet->payload_packet_len == 95
		    && get_u_int32_t(packet->payload, 0) == htonl(0x5f000000)
		    && get_u_int16_t(packet->payload, 4) == 0
		    && get_u_int16_t(packet->payload, 7) == htons(0x0004)
		    && get_u_int32_t(packet->payload, 20) == 0
		    && get_u_int32_t(packet->payload, 28) == htonl(0xc8000400)
		    && packet->payload[9] == 0x80
		    && get_u_int32_t(packet->payload,
				     10) == get_u_int32_t(packet->payload,
							  24)) {
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
				 NDPI_LOG_DEBUG, "found imesh.\n");
			ndpi_int_imesh_add_connection(ndpi_struct, flow,
		...
		if (packet->payload_packet_len == 28
		    && get_u_int32_t(packet->payload, 0) == htonl(0x1c000000)
		    && get_u_int16_t(packet->payload, 10) == htons(0xfcff)
		    && get_u_int32_t(packet->payload, 12) == htonl(0x07801800)
		    &&
		    (get_u_int16_t
		     (packet->payload,
		      packet->payload_packet_len - 2) == htons(0x1900)
		     || get_u_int16_t(packet->payload,
		...
		if ((packet->actual_payload_len == 8 || packet->payload_packet_len == 10)	/* PATTERN:: 04 00 00 00 00 00 00 00 [00 00] */
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
				 NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n",
			...
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
				 NDPI_LOG_DEBUG, "IMESH FOUND :: Payload %u\n",
				 packet->actual_payload_len);
		} else if (packet->actual_payload_len == 2
		...
		if (packet->payload_packet_len >
		    NDPI_STATICSTRING_LEN("POST /registration")
		    && memcmp(packet->payload, "POST /registration",
			      NDPI_STATICSTRING_LEN("POST /registration")) ==
		    0) {
			...
			if (packet->parsed_lines > 6 &&
			    ...
			    NDPI_STATICSTRING_LEN("login.bearshare.com")
			    && packet->line[1].ptr != NULL
			    && packet->line[1].len ==
			    NDPI_STATICSTRING_LEN("Authorization: Basic Og==")
			    && packet->line[4].ptr != NULL
			    && packet->line[4].len ==
			    NDPI_STATICSTRING_LEN("Accept-Encoding: identity")
			    && memcmp(packet->line[1].ptr,
				      "Authorization: Basic Og==",
				      NDPI_STATICSTRING_LEN
				      ("Authorization: Basic Og==")) == 0
			    && memcmp(packet->host_line.ptr,
				      "login.bearshare.com",
				      NDPI_STATICSTRING_LEN
				      ("login.bearshare.com")) == 0
		...
		if ((flow->l4.tcp.imesh_stage >= 4)
		    && (flow->l4.tcp.seen_syn && flow->l4.tcp.seen_syn_ack
			&& flow->l4.
			tcp.seen_ack) /* We have seen the 3-way handshake */ ) {
			NDPI_LOG(NDPI_PROTOCOL_IMESH, ndpi_struct,
				 NDPI_LOG_DEBUG, "found imesh.\n");
			ndpi_int_imesh_add_connection(ndpi_struct, flow,
						      NDPI_REAL_PROTOCOL);
