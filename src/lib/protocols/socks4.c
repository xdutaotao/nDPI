...
static void ndpi_int_socks4_add_connection(struct ndpi_detection_module_struct
					   *ndpi_struct,
					   struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_SOCKS4,
				NDPI_REAL_PROTOCOL);
}

static void ndpi_check_socks4(struct ndpi_detection_module_struct *ndpi_struct,
			      struct ndpi_flow_struct *flow)
{
	...
	if (flow->packet_counter > 20) {
		NDPI_LOG(NDPI_PROTOCOL_SOCKS4, ndpi_struct, NDPI_LOG_DEBUG,
			 "Exclude SOCKS4.\n");
	...
	if (flow->socks4_stage == 0) {
		...
		if ((payload_len == 9) && (((packet->payload[0] == 0x04)
					    ...
					    && (packet->payload[3] == 0x50))
					   || ((packet->payload[0] == 0x04)
					       && (packet->payload[1] == 0x01)
					       && (packet->payload[2] == 0x00)
					       && (packet->payload[3] ==
						   0x19)))) {
			NDPI_LOG(NDPI_PROTOCOL_SOCKS4, ndpi_struct,
				 NDPI_LOG_DEBUG,
