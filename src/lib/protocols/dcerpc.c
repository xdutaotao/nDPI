...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_DCERPC,
				NDPI_REAL_PROTOCOL);
}
			...
			struct ndpi_flow_struct *flow)
{
	...
	if ((packet->tcp != NULL)
	    ...
	    && ((ntohs(packet->tcp->source) == 135)
		|| (ntohs(packet->tcp->dest) == 135))
	    && (packet->payload[0] == 0x05)	/* version 5 */
	    &&(packet->payload[2] < 16)	/* Packet type */
	    ) {
		NDPI_LOG(NDPI_PROTOCOL_DCERPC, ndpi_struct, NDPI_LOG_DEBUG,
			 "DCERPC match\n");
