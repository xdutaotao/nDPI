...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_CORBA,
		       ...
		       struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_CORBA, ndpi_struct, NDPI_LOG_DEBUG,
		 "search for CORBA.\n");
	if (packet->tcp != NULL) {
		NDPI_LOG(NDPI_PROTOCOL_CORBA, ndpi_struct, NDPI_LOG_DEBUG,
			 "calculating CORBA over tcp.\n");
		/* Corba General Inter-ORB Protocol -> GIOP */
		if ((packet->payload_packet_len >= 24
		     && packet->payload_packet_len <= 144)
		    && memcmp(packet->payload, "GIOP", 4) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_CORBA, ndpi_struct,
				 NDPI_LOG_DEBUG, "found corba.\n");
