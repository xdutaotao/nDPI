...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_NETBIOS,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->udp != NULL) {
		...
		if ((dport == 137 || 0) && packet->payload_packet_len >= 50) {
			...
			NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "found netbios port 137 and payload_packet_len 50\n");
			...
			if (ntohs(get_u_int16_t(packet->payload, 2)) == 0 &&
				...
				NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "found netbios with questions = 1 and answers = 0, authority = 0  \n");
			...
			if (packet->payload[2] == 0x80 &&
			    ...
			    ntohs(get_u_int16_t(packet->payload, 6)) == 0 &&
			    ntohs(get_u_int16_t(packet->payload, 8)) == 0
			    && ntohs(get_u_int16_t(packet->payload, 10)) == 1) {

				NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "found netbios with questions = 1 and answers, authority, additional = 0  \n");
			...
			if ((ntohs(get_u_int16_t(packet->payload, 2)) & 0xf800)
			    == 0) {

				NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "possible netbios name query request\n");

				if (get_u_int16_t(packet->payload, 4) ==
				    htons(1)
			    ...
			    if ((ntohs(get_u_int16_t(packet->payload, 2)) &
				 0xf800) == 0x8000) {
				...
				} else if (get_u_int16_t(packet->payload, 4) ==
					   0
					   && get_u_int16_t(packet->payload,
							    6) == 0
					   && get_u_int16_t(packet->payload,
		...
		if ((dport == 138 ||
		    ...
		    ntohs(get_u_int16_t(packet->payload, 10)) ==
		    packet->payload_packet_len - 14) {

			NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "found netbios port 138 and payload length >= 112 \n");
			...
			if (packet->payload[0] >= 0x11
				...
				NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "found netbios with MSG-type 0x11,0x12,0x13,0x14,0x15 or 0x16\n");

				if (ntohl(get_u_int32_t(packet->payload, 4)) ==
				    ntohl(packet->iph->saddr)) {
					NDPI_LOG(NDPI_PROTOCOL_NETBIOS,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "found netbios with checked ip-address.\n");
	...
	if (packet->tcp != NULL) {
		...
		if (dport == 139) {
			...
			NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "found netbios with destination port 139\n");

			/* payload_packet_len must be 72 */
			if (packet->payload_packet_len == 72) {
				NDPI_LOG(NDPI_PROTOCOL_NETBIOS, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "found netbios with payload_packen_len = 72. \n");
