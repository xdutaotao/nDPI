...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_PPLIVE,
				NDPI_REAL_PROTOCOL);
}

static void ndpi_check_pplive_udp1(struct ndpi_detection_module_struct
				   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (flow->pplive_stage1 == 0) {
		...
		if ((payload_len > 0)
		    && match_first_bytes(packet->payload, "\xe9\x03\x41\x01")) {
		...
		if ((payload_len > 0)
		    && match_first_bytes(packet->payload, "\xe9\x03\x42\x01")) {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Possible PPLIVE request detected, we will look further for the response...\n");
		...
		if ((payload_len > 0)
		    && match_first_bytes(packet->payload, "\x1c\x1c\x32\x01")) {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Possible PPLIVE request detected, we will look further for the response...\n");
		...
		if ((payload_len > 0)
		    && (match_first_bytes(packet->payload, "\xe9\x03\x42\x01")
			|| match_first_bytes(packet->payload,
					     "\xe9\x03\x41\x01"))) {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found PPLIVE.\n");
			ndpi_int_pplive_add_connection(ndpi_struct, flow);
		} else {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "The reply did not seem to belong to PPLIVE, resetting the stage to 0...\n");
...
static void ndpi_check_pplive_udp2(struct ndpi_detection_module_struct
				   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	u_int32_t payload_len = packet->payload_packet_len;
...
static void ndpi_check_pplive_udp3(struct ndpi_detection_module_struct
				   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
	u_int32_t payload_len = packet->payload_packet_len;
	...
	if (flow->pplive_stage3 == 0) {
		...
		if ((payload_len == 94)
		    && (packet->udp->dest == htons(5041)
			|| packet->udp->source == htons(5041)
			|| packet->udp->dest == htons(8303)
			|| packet->udp->source == htons(8303))) {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Possible PPLIVE request detected, we will look further for the response...\n");
		...
		/* This is a packet in another direction. Check if we find the proper response. */
		if ((payload_len == 0) || (payload_len == 49)
		    || (payload_len == 94)) {
			NDPI_LOG(NDPI_PROTOCOL_PPLIVE, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found PPLIVE.\n");
			ndpi_int_pplive_add_connection(ndpi_struct, flow);
