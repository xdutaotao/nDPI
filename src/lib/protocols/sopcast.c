...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_SOPCAST,
				NDPI_REAL_PROTOCOL);
}
...
#endif
u_int8_t ndpi_int_is_sopcast_tcp(const u_int8_t * payload,
				 const u_int16_t payload_len)
{
	...
	if (payload[25] != payload[25 + 16 - 1] + 1
	    && payload[25] != payload[25 + 16 - 1] - 1) {

		if (payload[3] != payload[25] &&
		    payload[3] != payload[25] - 4
		    && payload[3] != payload[25] + 4
		    && payload[3] != payload[25] - 21) {
			return 0;
		}
	}
	...
	if (payload[4] != payload[28] ||
	    ...
	    get_u_int16_t(payload, 30) != get_u_int16_t(payload, 32)
	    || get_u_int16_t(payload, 32) != get_u_int16_t(payload, 34)) {

		if ((payload[2] != payload[5] - 1
		     && payload[2] != payload[5] + 1)
		    || payload[2] != payload[25] || payload[4] != payload[28]
		    || payload[4] != payload[31] || payload[4] != payload[32]
		    || payload[4] != payload[33] || payload[4] != payload[34]
		    || payload[4] != payload[35] || payload[4] != payload[30]
		    || payload[2] != payload[36]) {
			return 0;
		}
	}
	...
	if (payload[45] != payload[49] || payload[46] != payload[50]
	    || payload[47] != payload[51])
		return 0;
				    ...
				    *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (flow->packet_counter == 1 && packet->payload_packet_len == 54
	    && get_u_int16_t(packet->payload, 0) == ntohs(0x0036)) {
		if (ndpi_int_is_sopcast_tcp
		    (packet->payload, packet->payload_packet_len)) {
			NDPI_LOG(NDPI_PROTOCOL_SOPCAST, ndpi_struct,
				 NDPI_LOG_DEBUG, "found sopcast TCP \n");
			ndpi_int_sopcast_add_connection(ndpi_struct, flow);
			return;
		}
	}

	NDPI_LOG(NDPI_PROTOCOL_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG,
		 "exclude sopcast TCP.  \n");
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len == 42 && packet->payload[0] == 0x00
	    ...
	    && packet->payload[11] == 0x22 && packet->payload[12] == 0x00
	    && packet->payload[13] == 0x00) {
		NDPI_LOG(NDPI_PROTOCOL_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG,
			 "found sopcast with if IV.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	if (packet->payload_packet_len == 28 && packet->payload[0] == 0x00
	    ...
	    && packet->payload[9] == 0x01 && packet->payload[10] == 0x00
	    && packet->payload[11] == 0x14 && packet->payload[12] == 0x00
	    && packet->payload[13] == 0x00) {
		NDPI_LOG(NDPI_PROTOCOL_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG,
			 "found sopcast with if V.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
	/* this case has been seen once. Please revome this comment, if you see it another time */
	if (packet->payload_packet_len == 286 && packet->payload[0] == 0x00
	    ...
	    && packet->payload[11] == 0x16 && packet->payload[12] == 0x00
	    && packet->payload[13] == 0x00) {
		NDPI_LOG(NDPI_PROTOCOL_SOPCAST, ndpi_struct, NDPI_LOG_DEBUG,
			 "found sopcast with if VI.  \n");
		ndpi_int_sopcast_add_connection(ndpi_struct, flow);
		return;
	}
