...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_TDS,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len > 8
	    ...
	    && get_u_int16_t(packet->payload, 4) == 0x0000) {
		...
		if (flow->l4.tcp.tds_stage == 0) {
			if (packet->payload[0] != 0x02
				...
				flow->l4.tcp.tds_stage =
				    1 + packet->packet_direction;
				flow->l4.tcp.tds_login_version =
				    packet->payload[0];
				return;
			}
		} else if (flow->l4.tcp.tds_stage ==
			...
			case 0x12:
				if (packet->payload[0] == 0x12) {
					NDPI_LOG(NDPI_PROTOCOL_TDS, ndpi_struct,
						 NDPI_LOG_DEBUG,
						 "TDS detected\n");
