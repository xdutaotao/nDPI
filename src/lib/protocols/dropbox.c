...
static void ndpi_int_dropbox_add_connection(struct ndpi_detection_module_struct
					    *ndpi_struct,
...
static void ndpi_check_dropbox(struct ndpi_detection_module_struct *ndpi_struct,
			       struct ndpi_flow_struct *flow)
{
			...
			if (payload_len > 2) {
				if (strncmp
				    ((const char *)packet->payload, "{\"",
				     2) == 0) {
					NDPI_LOG(NDPI_PROTOCOL_DROPBOX,
	...
	NDPI_LOG(NDPI_PROTOCOL_DROPBOX, ndpi_struct, NDPI_LOG_DEBUG,
		 "exclude dropbox.\n");
