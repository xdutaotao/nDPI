			...
			struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_MEGACO, ndpi_struct, NDPI_LOG_DEBUG,
		 "search for MEGACO.\n");

	if (packet->udp != NULL) {
		if ((packet->payload_packet_len > 4 && packet->payload[0] == '!'
		     && packet->payload[1] == '/' && packet->payload[2] == '1'
			...
			&& packet->payload[8] == ' '
			&& packet->payload[9] == '[')) {
			NDPI_LOG(NDPI_PROTOCOL_MEGACO, ndpi_struct,
