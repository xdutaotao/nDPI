...
static void ndpi_int_filetopia_add_connection(struct
					      ndpi_detection_module_struct
					      *ndpi_struct,
					      struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_FILETOPIA,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->l4.tcp.filetopia_stage == 0) {
		if (packet->payload_packet_len >= 50
		    ...
		    && packet->payload[3] == 0x22
		    && packet->payload[packet->payload_packet_len - 1] ==
		    0x2b) {
			NDPI_LOG(NDPI_PROTOCOL_FILETOPIA, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Filetopia stage 1 detected\n");
		...
		if (packet->payload_packet_len >= 100
		    && packet->payload[0] == 0x03 && packet->payload[1] == 0x9a
		    && (packet->payload[3] == 0x22
			|| packet->payload[3] == 0x23)) {

			int i;
			for (i = 0; i < 10; i++) {	// check 10 bytes for valid ASCII printable characters
				if (!
				    (packet->payload[5 + i] >= 0x20
				     && packet->payload[5 + i] <= 0x7e)) {
		...
		if (packet->payload_packet_len >= 4
			...
			|| packet->payload[3] == 0x23)) {
			NDPI_LOG(NDPI_PROTOCOL_FILETOPIA, ndpi_struct,
				 NDPI_LOG_DEBUG, "Filetopia detected\n");
