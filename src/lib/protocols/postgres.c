...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_POSTGRES,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->l4.tcp.postgres_stage == 0) {
		...
		if (packet->payload_packet_len > 7 &&
		    ...
		    packet->payload[7] == 0x2f
		    && ntohl(get_u_int32_t(packet->payload, 0)) ==
		    packet->payload_packet_len) {
			flow->l4.tcp.postgres_stage =
		...
		if (flow->l4.tcp.postgres_stage == 2 - packet->packet_direction) {
			...
			if (packet->payload_packet_len == 1
			    && packet->payload[0] == 'S') {
				NDPI_LOG(NDPI_PROTOCOL_POSTGRES, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "PostgreSQL detected, SSL accepted.\n");
			...
			if (packet->payload_packet_len == 1
				...
				NDPI_LOG(NDPI_PROTOCOL_POSTGRES, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "PostgreSQL detected, SSL denied.\n");
				ndpi_int_postgres_add_connection(ndpi_struct,
								 flow);
				return;
			}
		...
		if (flow->l4.tcp.postgres_stage == 4 - packet->packet_direction)
			if (packet->payload_packet_len > 8 &&
			    ...
			    ntohl(get_u_int32_t(packet->payload, 1)) ==
			    packet->payload_packet_len - 1
			    && packet->payload[0] == 0x52) {
				NDPI_LOG(NDPI_PROTOCOL_POSTGRES, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "PostgreSQL detected, no SSL.\n");
				ndpi_int_postgres_add_connection(ndpi_struct,
								 flow);
				return;
			}
		if (flow->l4.tcp.postgres_stage == 6
			...
			NDPI_LOG(NDPI_PROTOCOL_POSTGRES, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "found postgres asymmetrically.\n");
			ndpi_int_postgres_add_connection(ndpi_struct, flow);
			return;
		}
		if (flow->l4.tcp.postgres_stage == 5
		    && packet->payload[0] == 'R') {
			if (ntohl(get_u_int32_t(packet->payload, 1)) ==
			    packet->payload_packet_len - 1) {
				NDPI_LOG(NDPI_PROTOCOL_POSTGRES, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "found postgres asymmetrically.\n");
			...
			size =
			    (u_int16_t) ntohl(get_u_int32_t(packet->payload, 1))
			    + 1;
			if (packet->payload[size - 1] == 'S') {
				if ((size +
				     get_u_int32_t(packet->payload,
						   (size + 1))) ==
				    packet->payload_packet_len) {
					NDPI_LOG(NDPI_PROTOCOL_POSTGRES,
