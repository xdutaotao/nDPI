...
static void ndpi_check_skype(struct ndpi_detection_module_struct *ndpi_struct,
			     struct ndpi_flow_struct *flow)
{
	...
	/*
	   ...
	   212.161.8.0/24
	 */
	if (((ntohl(packet->iph->saddr) & 0xFFFFFF00 /* 255.255.255.0 */ ) ==
	     0xD4A10800 /* 212.161.8.0 */ )
	    || ((ntohl(packet->iph->daddr) & 0xFFFFFF00 /* 255.255.255.0 */ ) ==
		0xD4A10800 /* 212.161.8.0 */ )
	    /* || is_skype_connection(ndpi_struct, packet->iph->saddr, packet->iph->daddr) */
	    ) {
		ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_SKYPE,
					NDPI_REAL_PROTOCOL);
		return;
	}
	...
	if (packet->udp != NULL) {
		...
		if (flow->l4.udp.skype_packet_id < 5) {
			/* skype-to-skype */
			if (((payload_len == 3)
			     && ((packet->payload[2] & 0x0F) == 0x0d))
			    || ((payload_len >= 16)
				&& (packet->payload[0] != 0x30)	/* Avoid invalid SNMP detection */
				&&(packet->payload[2] == 0x02))) {
				NDPI_LOG(NDPI_PROTOCOL_SKYPE, ndpi_struct,
					 NDPI_LOG_DEBUG, "Found skype.\n");
		       ...
		       struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_SKYPE, ndpi_struct, NDPI_LOG_DEBUG,
		 "skype detection...\n");
