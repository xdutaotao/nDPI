...
static void ndpi_int_edonkey_add_connection(struct ndpi_detection_module_struct
					    *ndpi_struct,
					    struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_EDONKEY,
				NDPI_REAL_PROTOCOL);
}
...
static int ndpi_edonkey_payload_check(const u_int8_t * data, u_int32_t len)
{
	...
	if ((len >= 4) && (data[0] == 0xe3) && (data[2] == 0x00)
	    && (data[3] == 0x00))
		return 1;

	if ((len >= 4) && (data[0] == 0xc5) && (data[2] == 0x00)
	    && (data[3] == 0x00))
		return 1;
	...
	if ((len >= 4) && (data[0] == 0xe5) && (data[1] == 0x08)
	    && (data[2] == 0x78) && (data[3] == 0xda))
		return 1;

	if ((len >= 4) && (data[0] == 0xe5) && (data[1] == 0x28)
	    && (data[2] == 0x78) && (data[3] == 0xda))
		return 1;
	...
	if ((len <= 34 && ((len - 2) % 4 == 0)) && (data[0] == 0xe3)
	    && (data[1] == 0x97))
		return 1;
	...
	if ((len == 18) && (data[0] == 0xe4) && (data[1] == 0x01)
	...
	if ((len == 22 || len == 38 || len == 28) && (data[0] == 0xe4)
	    && (data[1] == 0x19))
		return 1;
	...
	if ((len == 119 || len == 69 || len == 294) && (data[0] == 0xe4)
	    && (data[1] == 0x29))
		return 1;

	if ((len == 119 || len == 69 || len == 294 || len == 44 || len == 269)
	    && (data[0] == 0xe4) && (data[1] == 0x28))
		return 1;

	return 0;
}

static void ndpi_check_edonkey(struct ndpi_detection_module_struct *ndpi_struct,
			       struct ndpi_flow_struct *flow)
{
	...
	if (flow->packet_counter > 20) {
		NDPI_LOG(NDPI_PROTOCOL_EDONKEY, ndpi_struct, NDPI_LOG_DEBUG,
			 "Exclude EDONKEY.\n");
	...
	if (flow->edonkey_stage == 0) {
		...
		if (ndpi_edonkey_payload_check(packet->payload, payload_len)) {
			NDPI_LOG(NDPI_PROTOCOL_EDONKEY, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "Possible EDONKEY request detected, we will look further for the response...\n");
		...
		if ((payload_len == 0)
		    ||
		    (ndpi_edonkey_payload_check(packet->payload, payload_len)))
		{
			NDPI_LOG(NDPI_PROTOCOL_EDONKEY, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found EDONKEY.\n");
			ndpi_int_edonkey_add_connection(ndpi_struct, flow);
		} else {
			NDPI_LOG(NDPI_PROTOCOL_EDONKEY, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "The reply did not seem to belong to EDONKEY, resetting the stage to 0...\n");
