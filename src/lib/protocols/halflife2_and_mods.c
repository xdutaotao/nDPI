...
static void ndpi_int_halflife2_add_connection(struct
					      ndpi_detection_module_struct
					      *ndpi_struct,
					      struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_HALFLIFE2,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (flow->l4.udp.halflife2_stage == 0) {
		if (packet->payload_packet_len >= 20
		    ...
		    htonl(0x30303000)) {
			flow->l4.udp.halflife2_stage =
			    1 + packet->packet_direction;
			NDPI_LOG(NDPI_PROTOCOL_HALFLIFE2, ndpi_struct,
				 NDPI_LOG_DEBUG,
		...
		if (packet->payload_packet_len >= 20
			...
			NDPI_LOG(NDPI_PROTOCOL_HALFLIFE2, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "halflife2 server reply detected\n");
			return;
		}
	}
