...
static void ndpi_check_radius(struct ndpi_detection_module_struct *ndpi_struct,
			      struct ndpi_flow_struct *flow)
{
	...
	if (packet->udp != NULL) {
		struct radius_header *h =
		    (struct radius_header *)packet->payload;
		...
		if ((payload_len > sizeof(struct radius_header))
		    ...
		    && (h->code <= 5)
		    && (len == payload_len)) {
			NDPI_LOG(NDPI_PROTOCOL_RADIUS, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found radius.\n");
			...
			struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_RADIUS, ndpi_struct, NDPI_LOG_DEBUG,
		 "radius detection...\n");
