...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_STEALTHNET,
				NDPI_REAL_PROTOCOL);
}
...
//  struct ndpi_id_struct *dst = flow->dst;
	...
	if (packet->payload_packet_len > 40
	    && memcmp(packet->payload,
		      "LARS REGENSBURGER'S FILE SHARING PROTOCOL", 41) == 0) {
		NDPI_LOG(NDPI_PROTOCOL_STEALTHNET, ndpi_struct, NDPI_LOG_DEBUG,
			 "found stealthnet\n");
