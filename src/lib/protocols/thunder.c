					    ...
					    ndpi_protocol_type_t protocol_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_THUNDER,
				protocol_type);
				 ...
				 *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->payload_packet_len > 8 && packet->payload[0] >= 0x30
	    && packet->payload[0] < 0x40 && packet->payload[1] == 0
		...
		if (flow->thunder_stage == 2) {
			...
			NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "THUNDER udp detected. Well Done! \n");
	...
	if (flow->init_finished
		...
		NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct, NDPI_LOG_DEBUG,
			 "correlated THUNDER udp detected. Well Done! \n");
		ndpi_int_thunder_add_connection(ndpi_struct, flow,
						NDPI_REAL_PROTOCOL);
		return;
	}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len > 8 && packet->payload[0] >= 0x30
		...
		if (flow->thunder_stage == 3) {
			NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct,
				 NDPI_LOG_DEBUG, "THUNDER tcp detected\n");
			ndpi_int_thunder_add_connection(ndpi_struct, flow,
							NDPI_REAL_PROTOCOL);
			return;
		}
	...
	if (flow->thunder_stage == 0 && packet->payload_packet_len > 17
		...
		NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct, NDPI_LOG_DEBUG,
			 ...
			 packet->parsed_lines, packet->empty_line_position_set,
				  ...
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_THUNDER) {
		if (src != NULL && ((u_int32_t)
				    (packet->tick_timestamp - src->thunder_ts) <
				    ndpi_struct->thunder_timeout)) {
			NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct,
				 NDPI_LOG_DEBUG,
	...
	if (packet->payload_packet_len > 5
	    ...
	    && NDPI_SRC_OR_DST_HAS_PROTOCOL(src, dst, NDPI_PROTOCOL_THUNDER)) {
		NDPI_LOG(NDPI_PROTOCOL_THUNDER, ndpi_struct, NDPI_LOG_DEBUG,
