...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_IAX,
				NDPI_REAL_PROTOCOL);
}
				  ...
				  *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (			/* 1. iax is udp based, port 4569 */
		   (packet->udp->source == htons(4569)
		    || packet->udp->dest == htons(4569))
		...
		if (packet->payload_packet_len == 12) {
			NDPI_LOG(NDPI_PROTOCOL_IAX, ndpi_struct, NDPI_LOG_DEBUG,
				 "found IAX.\n");
		...
		for (i = 0; i < NDPI_IAX_MAX_INFORMATION_ELEMENTS; i++) {
			...
			if (packet_len == packet->payload_packet_len) {
				NDPI_LOG(NDPI_PROTOCOL_IAX, ndpi_struct,
					 NDPI_LOG_DEBUG, "found IAX.\n");
				ndpi_int_iax_add_connection(ndpi_struct, flow);
				return;
			}
			if (packet_len > packet->payload_packet_len) {
