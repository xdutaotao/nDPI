...
static void ndpi_add_connection_as_bittorrent(struct
						    ...
						    *flow)
{
	...
	if (packet->payload_packet_len > 20) {
		...
		if (packet->payload[0] == 0x13) {
			if (memcmp
			    (&packet->payload[1], "BitTorrent protocol",
			     19) == 0) {
	...
	if (packet->payload_packet_len > 23
	    && memcmp(packet->payload, "GET /webseed?info_hash=", 23) == 0) {
		NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct, NDPI_LOG_TRACE,
			 "BT: plain webseed BitTorrent protocol detected\n");
		ndpi_add_connection_as_bittorrent(ndpi_struct, flow,
						  NDPI_PROTOCOL_SAFE_DETECTION,
						  NDPI_PROTOCOL_WEBSEED_DETECTION,
	...
	if (packet->payload_packet_len > 60
		...
		NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct, NDPI_LOG_TRACE,
			 "BT: plain Bitcomet persistent seed protocol detected\n");
		ndpi_add_connection_as_bittorrent(ndpi_struct, flow,
						  NDPI_PROTOCOL_SAFE_DETECTION,
						  NDPI_PROTOCOL_WEBSEED_DETECTION,
						  NDPI_CORRELATED_PROTOCOL);
	...
	if (packet->payload_packet_len > 90
		...
		if (packet->user_agent_line.ptr != NULL
		    &&
		    ((packet->user_agent_line.len > 8
		      && memcmp(packet->user_agent_line.ptr, "Azureus ",
				8) == 0)
		     || (packet->user_agent_line.len >= 10
			 && memcmp(packet->user_agent_line.ptr, "BitTorrent",
				   10) == 0)
		     || (packet->user_agent_line.len >= 11
			 && memcmp(packet->user_agent_line.ptr, "BTWebClient",
				   11) == 0))) {
			NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct,
				 NDPI_LOG_TRACE,
				 "Azureus /Bittorrent user agent line detected\n");
			ndpi_add_connection_as_bittorrent(ndpi_struct, flow,
							  NDPI_PROTOCOL_SAFE_DETECTION,
							  NDPI_PROTOCOL_WEBSEED_DETECTION,
							  NDPI_CORRELATED_PROTOCOL);
		...
		if (packet->user_agent_line.ptr != NULL
		    && (packet->user_agent_line.len >= 9
			&& memcmp(packet->user_agent_line.ptr, "Shareaza ",
				  9) == 0)
		    && (packet->parsed_lines > 8 && packet->line[8].ptr != 0
			&& packet->line[8].len >= 9
			&& memcmp(packet->line[8].ptr, "X-Queue: ", 9) == 0)) {
			NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct,
				 NDPI_LOG_TRACE,
				 "Bittorrent Shareaza detected.\n");
			ndpi_add_connection_as_bittorrent(ndpi_struct, flow,
							  NDPI_PROTOCOL_SAFE_DETECTION,
							  NDPI_PROTOCOL_WEBSEED_DETECTION,
							  NDPI_CORRELATED_PROTOCOL);
		...
		if ((packet->parsed_lines == 10
		    ...
		    && memcmp(packet->user_agent_line.ptr, "Mozilla/4.0 ",
			      12) == 0 && packet->host_line.ptr != NULL
		    && packet->host_line.len >= 7 && packet->line[2].ptr != NULL
		    && packet->line[2].len > 14
		    && memcmp(packet->line[2].ptr, "Keep-Alive: 300", 15) == 0
		    && packet->line[3].ptr != NULL && packet->line[3].len > 21
		    && memcmp(packet->line[3].ptr, "Connection: Keep-alive",
			      22) == 0 && packet->line[4].ptr != NULL
		    ...
		    && packet->line[8].len > 22
		    && memcmp(packet->line[8].ptr, "Cache-Control: no-cache",
			      23) == 0) {

			NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct,
				 NDPI_LOG_TRACE, "Bitcomet LTS detected\n");
			ndpi_add_connection_as_bittorrent(ndpi_struct, flow,
		...
		if (packet->parsed_lines == 8
		    ...
		    (sizeof("Mozilla/4.0 (compatible; MSIE 6.0;") - 1)
		    && memcmp(packet->user_agent_line.ptr,
			      "Mozilla/4.0 (compatible; MSIE 6.0;",
			      sizeof("Mozilla/4.0 (compatible; MSIE 6.0;") -
			      1) == 0 && packet->host_line.ptr != NULL
		    && packet->host_line.len >= 7 && packet->line[2].ptr != NULL
		    && packet->line[2].len == 11
		    && memcmp(packet->line[2].ptr, "Accept: */*", 11) == 0
		    && packet->line[3].ptr != NULL
		    && packet->line[3].len >= (sizeof("Referer: ") - 1)
		    && memcmp(packet->line[3].ptr, "Referer: ",
			      sizeof("Referer: ") - 1) == 0
		    && packet->line[5].ptr != NULL && packet->line[5].len > 13
		    && memcmp(packet->line[5].ptr, "Range: bytes=", 13) == 0
		    && packet->line[6].ptr != NULL && packet->line[6].len > 21
		    && memcmp(packet->line[6].ptr, "Connection: Keep-Alive",
			      22) == 0) {
		...
		if (packet->parsed_lines == 7
		    ...
		    && memcmp(packet->line[3].ptr, "Referer: ",
			      sizeof("Referer: ") - 1) == 0
		    && packet->line[5].ptr != NULL && packet->line[5].len > 21
		    && memcmp(packet->line[5].ptr, "Connection: Keep-Alive",
			      22) == 0) {
		...
		NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct,
			 NDPI_LOG_TRACE,
			 " BT stat: tracker info hash parsed\n");
		ndpi_add_connection_as_bittorrent(ndpi_struct, flow,
						  NDPI_PROTOCOL_SAFE_DETECTION,
						  NDPI_PROTOCOL_PLAIN_DETECTION,
						  NDPI_CORRELATED_PROTOCOL);
...
ndpi_end_bt_tracker_check:
	...
	if (packet->payload_packet_len == 80) {
		 ...
		 * */
		static const char pattern_20_bytes[20] =
		    { 0x4c, 0x00, 0x00, 0x00, 0xff,
			0xff, 0xff, 0xff, 0x57, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00,
			0x00
		};
		static const char pattern_12_bytes[12] =
		    { 0x28, 0x23, 0x00, 0x00, 0x01,
		...
		if (memcmp(packet->payload, "GET", 3) == 0) {
			...
			if (packet->host_line.ptr != NULL
			    ...
			    && memcmp(packet->host_line.ptr, "ip2p.com:",
				      9) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_BITTORRENT, ndpi_struct,
					 NDPI_LOG_TRACE,
			    ...
			    struct ndpi_flow_struct *flow)
{
	...
	/* This is broadcast */
	if (packet->iph && ((packet->iph->saddr == 0xFFFFFFFF)
	...
	if (packet->detected_protocol_stack[0] != NDPI_PROTOCOL_BITTORRENT) {
		...
		if ((packet->tcp != NULL)
			...
			if (packet->payload_packet_len >=
				...
				u_int32_t v1_window_size =
				    *((u_int32_t *) & packet->payload[12]);
...
bittorrent_found:
						...
						while ((packet->payload_packet_len - 19) > offset && (begin = memchr(begin, 'B', packet->payload_packet_len - 19)) != NULL) {
							offset =
							    (u_long) begin -
							    (u_long)
							    packet->payload;
