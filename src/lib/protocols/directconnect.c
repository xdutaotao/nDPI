...
static u_int32_t skip_unknown_headers(const u_int8_t * payload,
				      u_int32_t payload_len, u_int32_t pos)
{
	u_int32_t i = pos;
...
static u_int16_t parse_binf_message(struct ndpi_detection_module_struct
				    *ndpi_struct, const u_int8_t * payload,
				    int payload_len)
{
	u_int32_t i = 4;
			...
			if (memcmp(&payload[i], "DCTM", 4) == 0) {
				if (memcmp(&payload[i + 15], "ADCS", 4) == 0) {
					ssl_port =
					    ...
					    (&payload[i + 25], 5, &bytes_read);
					NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
						 ndpi_struct, NDPI_LOG_DEBUG,
						 "directconnect ssl port parsed %d",
						 ssl_port);
...
static void ndpi_int_directconnect_add_connection(struct
						  ...
						  connection_type)
{
	...
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_DIRECTCONNECT,
				NDPI_REAL_PROTOCOL);
	...
	if (src != NULL) {
		...
		if (connection_type == DIRECT_CONNECT_TYPE_PEER) {
			if (packet->tcp != NULL
			    && flow->setup_packet_direction !=
			    packet->packet_direction
			    && src->detected_directconnect_port == 0) {
				src->detected_directconnect_port =
				    packet->tcp->source;
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "directconnect tcp PORT %u for src\n",
					 ntohs
					 (src->detected_directconnect_port));
			}
			if (packet->udp != NULL
			    && src->detected_directconnect_udp_port == 0) {
				src->detected_directconnect_udp_port =
				    packet->udp->source;
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "directconnect udp PORT %u for src\n",
					 ntohs
					 (src->detected_directconnect_port));
					  ...
					  struct ndpi_flow_struct *flow)
{
	...
	if (flow->detected_protocol_stack[0] == NDPI_PROTOCOL_DIRECTCONNECT) {
		if (packet->payload_packet_len >= 40
		    && memcmp(&packet->payload[0], "BINF", 4) == 0) {
			u_int16_t ssl_port = 0;
			ssl_port =
			    parse_binf_message(ndpi_struct, &packet->payload[4],
		...
		if ((packet->payload_packet_len >= 38
		    ...
		    && memcmp(&packet->payload[0], "DCTM", 4) == 0
		    && memcmp(&packet->payload[15], "ADCS", 4) == 0) {
			u_int16_t bytes_read = 0;
			if (dst != NULL) {
				dst->detected_directconnect_ssl_port =
				    ntohs_ndpi_bytestream_to_number
				    (&packet->payload[25], 5, &bytes_read);
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "directconnect ssl port parsed %d",
					 ntohs
	...
	if (src != NULL) {
		if (src->detected_directconnect_port == packet->tcp->source) {
			    ...
			    ndpi_struct->directconnect_connection_ip_tick_timeout)
			{
				...
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "resetting src port due to timeout");
				return;
			}
		}
		...
		if (dst->detected_directconnect_port == packet->tcp->dest) {
			    ...
			    ndpi_struct->directconnect_connection_ip_tick_timeout)
			{
				...
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ...
					 "marking using dc port\n %d",
					 ntohs
					 (dst->detected_directconnect_port));
				return;
			} else {
				dst->detected_directconnect_port = 0;
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "resetting dst port due to timeout");
				return;
			}
		}
	...
	if (flow->directconnect_stage == 0) {
		...
		if (packet->payload_packet_len > 6) {
			...
			if (packet->payload_packet_len > 7
			    ...
			    && packet->payload[packet->payload_packet_len -
					       1] == '|'
			    && (memcmp(&packet->payload[1], "MyNick ", 7) ==
				0)) {
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "maybe first dc connect between peers  detected\n");
		...
		if (packet->payload_packet_len >= 11) {
			...
			if (memcmp(&packet->payload[0], "HSUP ADBAS0", 11) == 0
			    || memcmp(&packet->payload[0], "HSUP ADBASE",
				      11) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "found directconnect HSUP ADBAS0 E\n");
				ndpi_int_directconnect_add_connection
				    (ndpi_struct, flow,
				     DIRECT_CONNECT_TYPE_HUB);
				return;
				/* did not see this pattern in any trace */
			} else
			    if (memcmp(&packet->payload[0], "CSUP ADBAS0", 11)
				== 0
				|| memcmp(&packet->payload[0], "CSUP ADBASE",
					  11) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "found directconnect CSUP ADBAS0 E\n");
				ndpi_int_directconnect_add_connection
				    (ndpi_struct, flow,
				     DIRECT_CONNECT_ADC_PEER);
				return;
		...
		if (packet->payload_packet_len >= 11) {
			...
			if (memcmp(&packet->payload[0], "HSUP ADBAS0", 11) == 0
				...
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "found directconnect HSUP ADBAS E in second packet\n");
				ndpi_int_directconnect_add_connection
				    (ndpi_struct, flow,
				     DIRECT_CONNECT_TYPE_HUB);
				...
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "found directconnect HSUP ADBAS0 E in second packet\n");
				ndpi_int_directconnect_add_connection
				    (ndpi_struct, flow,
				     DIRECT_CONNECT_ADC_PEER);
		...
		if (packet->payload_packet_len > 6) {
			if (packet->payload[0] == '$'
				...
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "second dc between peers detected\n");

				ndpi_int_directconnect_add_connection
				    (ndpi_struct, flow,
				     DIRECT_CONNECT_TYPE_PEER);

				return;
			} else {
				NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT,
					 ndpi_struct, NDPI_LOG_DEBUG,
					 "second dc between peers not detected\n");
			}
		}
					  ...
					  struct ndpi_flow_struct *flow)
{
	...
	if (dst != NULL
	    && dst->detected_directconnect_udp_port == packet->udp->dest) {
		if ((u_int32_t)
		    (packet->tick_timestamp -
		     dst->directconnect_last_safe_access_time) <
		    ndpi_struct->directconnect_connection_ip_tick_timeout) {
			...
			NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "resetting dst udp  port due to timeout");
			return;
		}
	}
			       ...
			       *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->detected_protocol_stack[0] == NDPI_PROTOCOL_DIRECTCONNECT) {
					   ...
					   ndpi_struct->directconnect_connection_ip_tick_timeout))
		{
			...
			NDPI_LOG(NDPI_PROTOCOL_DIRECTCONNECT, ndpi_struct,
				 NDPI_LOG_DEBUG,
				 "directconnect: skipping as unknown due to timeout\n");
		}
		return;
	}
