...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_MSN,
				protocol_type);
}

static u_int8_t ndpi_int_find_xmsn(struct ndpi_detection_module_struct
				   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	if (packet->parsed_lines > 3) {
		...
		for (i = 2; i < packet->parsed_lines; i++) {
			if (packet->line[i].ptr != NULL
			    && packet->line[i].len >
			    NDPI_STATICSTRING_LEN("X-MSN")
			    && memcmp(packet->line[i].ptr, "X-MSN",
				      NDPI_STATICSTRING_LEN("X-MSN")) == 0) {
				return 1;
			}
		}
	}
...
static void ndpi_search_msn_tcp(struct ndpi_detection_module_struct
				*ndpi_struct, struct ndpi_flow_struct *flow)
{
	struct ndpi_packet_struct *packet = &flow->packet;
		...
		if (flow->packet_counter == 7
			...
			if (memcmp(packet->payload + 24, "MSNSLP", 6) == 0
				...
				NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct,
					 NDPI_LOG_TRACE,
					 "detected MSN File Transfer, ifdef ssl.\n");
	...
	if (flow->packet_counter == 1
		...
		if (packet->payload_packet_len > 32
		    ...
		    ((ntohl(get_u_int32_t(packet->payload, 24)) == 0x000f0004
		      && ntohl(get_u_int32_t(packet->payload, 28)) ==
		      0x72c64bc6)
		...
		if (
			   ...
			   NDPI_PROTOCOL_HTTP ||
#endif
			   memcmp(packet->payload, "GET ",
				  NDPI_STATICSTRING_LEN("GET ")) == 0
			   || memcmp(packet->payload, "POST ",
				     NDPI_STATICSTRING_LEN("POST ")) == 0) {
			ndpi_parse_packet_line_info(ndpi_struct, flow);
			if (packet->user_agent_line.ptr != NULL &&
			    packet->user_agent_line.len >
			    NDPI_STATICSTRING_LEN("Messenger/")
			    && memcmp(packet->user_agent_line.ptr, "Messenger/",
				      NDPI_STATICSTRING_LEN("Messenger/")) ==
			    0) {
				ndpi_int_msn_add_connection(ndpi_struct, flow,
							    NDPI_CORRELATED_PROTOCOL);
		...
		if (packet->payload_packet_len > 100) {
			if (
				   ...
				   NDPI_PROTOCOL_HTTP ||
#endif
				   memcmp(packet->payload, "POST http://",
					  12) == 0) {
				...
				if (packet->content_line.ptr != NULL &&
				    ((packet->content_line.len ==
				      ...
				      && memcmp(packet->content_line.ptr,
						"application/x-msn-messenger",
						NDPI_STATICSTRING_LEN
						("application/x-msn-messenger"))
				      == 0)
				     || (packet->content_line.len >=
					 NDPI_STATICSTRING_LEN("text/x-msnmsgr")
					 && memcmp(packet->content_line.ptr,
						   "text/x-msnmsgr",
						   NDPI_STATICSTRING_LEN
						   ("text/x-msnmsgr")) == 0))) {
					...
					ndpi_int_msn_add_connection(ndpi_struct,
								    flow,
								    NDPI_CORRELATED_PROTOCOL);
					return;
				}
		...
		if (packet->payload_packet_len > 400) {
			if ((
				...
				u_int16_t c;
				if (memcmp(&packet->payload[5], "http://", 7) ==
				    0) {
					/*
		...
		if (flow->packet_counter <= 10 &&
		    (flow->packet_direction_counter[0] <= 2
		     || flow->packet_direction_counter[1] <= 2)
		    && packet->payload_packet_len > 100) {
...
#endif
				   (memcmp
				    (packet->payload, "HTTP/1.0 200 OK",
				     15) == 0)
				   ||
				   (memcmp
				    (packet->payload, "HTTP/1.1 200 OK",
				     15) == 0)
			    ) {
		...
		if (packet->payload_packet_len >= 42) {
			if (memcmp
			    (packet->payload,
			     "CONNECT messenger.hotmail.com:1863 HTTP/1.",
			     42) == 0) {
				NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct,
					 NDPI_LOG_TRACE,
		...
		if (packet->payload_packet_len >= 18) {
			...
			if (memcmp(packet->payload, "USR ", 4) == 0
			    || memcmp(packet->payload, "ANS ", 4) == 0) {
				/* now we must see a number */
				const u_int16_t endlen =
				...
				while (1) {
					...
					if (packet->payload[plen] < '0'
					    || packet->payload[plen] > '9') {
				...
				while (plen < endlen) {
					if (ndpi_check_for_email_address
					    (ndpi_struct, flow, plen) != 0) {
						NDPI_LOG(NDPI_PROTOCOL_MSN,
							 ndpi_struct,
							 NDPI_LOG_TRACE,
							 "found mail address\n");
	...
	if (flow->packet_counter <= 10 &&
		...
		if (
			   ...
			   || (memcmp(packet->payload, "HTTP/1.1 200 OK", 15) ==
			       0)
		    ) {
	...
	if ((src != NULL
		...
		NDPI_COMPARE_PROTOCOL_TO_BITMASK(dst->detected_protocol_bitmask,
						 NDPI_PROTOCOL_MSN) != 0)) {
		if (flow->packet_counter == 1 && packet->payload_packet_len > 12
		    && memcmp(packet->payload, "recipientid=", 12) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_DEBUG,
				 "detected file transfer.\n");
			ndpi_int_msn_add_connection(ndpi_struct, flow,
						    NDPI_REAL_PROTOCOL);
			return;
		}
	...
	if (flow->l4.tcp.msn_stage == 0) {
		...
		if ((packet->payload_packet_len == 4
			...
			if (packet->payload_packet_len == 8
			    && get_u_int32_t(packet->payload,
					     4) == htonl(0x666f6f00)) {
				flow->l4.tcp.msn_stage =
				    5 + packet->packet_direction;
				return;
			}
		...
		if (packet->payload_packet_len > 10
		    && get_u_int32_t(packet->payload, 0) == htonl(0x666f6f00)) {
			ndpi_int_msn_add_connection(ndpi_struct, flow,
						    NDPI_REAL_PROTOCOL);
			NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE,
				 "MSN File Transfer detected 1\n");
			return;
		}
		/* did not see this pattern in any trace */
		if (packet->payload_packet_len == 56
		    && get_u_int32_t(packet->payload, 16) == 0) {
			NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_DEBUG,
				 "maybe Second TCP MSN detected\n");
		...
		if ((packet->payload_packet_len == 4) &&
		    (get_u_int32_t(packet->payload, 0) == htonl(0x10000000)
		     || get_u_int32_t(packet->payload, 0) == htonl(0x30000000))) {
			ndpi_int_msn_add_connection(ndpi_struct, flow,
						    NDPI_REAL_PROTOCOL);
			NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE,
				 "MSN File Transfer detected 3\n");
			return;
		}
	...
	if (flow->packet_counter <= MAX_PACKETS_FOR_MSN) {
		if (packet->tcp->source == htons(443)
			...
			if (packet->payload_packet_len > 300) {
				if (memcmp
				    (&packet->payload[40], "INVITE MSNMSGR",
				     14) == 0
				    || memcmp(&packet->payload[56],
					      "INVITE MSNMSGR", 14) == 0
				    || memcmp(&packet->payload[172],
					      "INVITE MSNMSGR", 14) == 0) {
					ndpi_int_msn_add_connection(ndpi_struct,
								    flow,
								    NDPI_REAL_PROTOCOL);
				     ...
				     struct ndpi_flow_struct *flow)
{
	...
	if (packet->payload_packet_len == 20
		...
		NDPI_LOG(NDPI_PROTOCOL_MSN, ndpi_struct, NDPI_LOG_TRACE,
			 "msn udp misc data connection detected\n");
		ndpi_int_msn_add_connection(ndpi_struct, flow,
					    NDPI_REAL_PROTOCOL);
	}
		     ...
		     struct ndpi_flow_struct *flow)
{
	...
	if (NDPI_COMPARE_PROTOCOL_TO_BITMASK
	    (flow->excluded_protocol_bitmask, NDPI_PROTOCOL_MSN) == 0) {
