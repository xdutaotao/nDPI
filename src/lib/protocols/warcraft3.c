...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_WARCRAFT3,
				NDPI_REAL_PROTOCOL);
}
			   ...
			   *ndpi_struct, struct ndpi_flow_struct *flow)
{
	...
	NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3, ndpi_struct, NDPI_LOG_DEBUG,
		 "search WARCRAFT3\n");

	if (flow->packet_counter == 1 && packet->payload_packet_len == 1
	    && packet->payload[0] == 0x01) {
		NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3, ndpi_struct, NDPI_LOG_DEBUG,
			 "maybe warcraft3: packet_len == 1\n");
		return;
	} else if (packet->payload_packet_len >= 4
		   && (packet->payload[0] == 0xf7
		       || packet->payload[0] == 0xff)) {

		NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3, ndpi_struct, NDPI_LOG_DEBUG,
			 "packet_payload begins with 0xf7 or 0xff\n");
		...
		while (l <= (packet->payload_packet_len - 4)) {
			if (packet->payload[l] == 0xf7) {
				u_int16_t temp =
				    (packet->payload[l + 2 + 1] << 8) +
				    packet->payload[l + 2];
				NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3, ndpi_struct,
					 NDPI_LOG_DEBUG,
					 "another f7 visited.\n");

				if ((temp <= 2) || (temp > 1500)) {
					NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3,
		...
		if (l == packet->payload_packet_len) {
			NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3, ndpi_struct,
				 NDPI_LOG_DEBUG, "maybe WARCRAFT3\n");
			NDPI_LOG(NDPI_PROTOCOL_WARCRAFT3, ndpi_struct,
				 NDPI_LOG_DEBUG, "flow->packet_counter = %u \n",
				 flow->packet_counter);
