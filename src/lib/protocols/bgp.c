...
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_BGP,
				NDPI_REAL_PROTOCOL);
}
...
//      struct ndpi_id_struct         *dst=ndpi_struct->dst;
	...
	if (packet->payload_packet_len > 18 &&
	    ...
	    ntohs(get_u_int16_t(packet->payload, 16)) <=
	    packet->payload_packet_len && (packet->tcp->dest == htons(179)
					   || packet->tcp->source == htons(179))
	    && packet->payload[18] < 5) {
		NDPI_LOG(NDPI_PROTOCOL_BGP, ndpi_struct, NDPI_LOG_DEBUG,
			 "BGP detected.\n");
