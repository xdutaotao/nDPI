...
static void ndpi_int_socks5_add_connection(struct ndpi_detection_module_struct
					   *ndpi_struct,
					   struct ndpi_flow_struct *flow)
{
	ndpi_int_add_connection(ndpi_struct, flow, NDPI_PROTOCOL_SOCKS5,
				NDPI_REAL_PROTOCOL);
}

static void ndpi_check_socks5(struct ndpi_detection_module_struct *ndpi_struct,
			      struct ndpi_flow_struct *flow)
{
	...
	if (flow->packet_counter > 20) {
		NDPI_LOG(NDPI_PROTOCOL_SOCKS5, ndpi_struct, NDPI_LOG_DEBUG,
			 "Exclude SOCKS5.\n");
	...
	if (flow->socks5_stage == 0) {
		...
		if ((payload_len == 3) && (packet->payload[0] == 0x05)
		    && (packet->payload[1] == 0x01)
		    && (packet->payload[2] == 0x00)) {
			NDPI_LOG(NDPI_PROTOCOL_SOCKS5, ndpi_struct,
				 NDPI_LOG_DEBUG,
		...
		if ((payload_len == 0)
		    || ((payload_len == 2) && (packet->payload[0] == 0x05)
			&& (packet->payload[1] == 0x00))) {
			NDPI_LOG(NDPI_PROTOCOL_SOCKS5, ndpi_struct,
				 NDPI_LOG_DEBUG, "Found SOCKS5.\n");
