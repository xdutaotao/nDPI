...
static void ndpi_int_activesync_add_connection(struct
			    ...
			    struct ndpi_flow_struct *flow)
{
	...
	if (packet->tcp != NULL) {
		...
		if (packet->payload_packet_len > 150
		    &&
		    ((memcmp
		      (packet->payload, "OPTIONS /Microsoft-Server-ActiveSync?",
		       37) == 0)
		     ||
		     (memcmp
		      (packet->payload, "POST /Microsoft-Server-ActiveSync?",
		       34) == 0))) {
			ndpi_int_activesync_add_connection(ndpi_struct, flow);
			NDPI_LOG(NDPI_PROTOCOL_HTTP_APPLICATION_ACTIVESYNC,
	...
	NDPI_LOG(NDPI_PROTOCOL_HTTP_APPLICATION_ACTIVESYNC, ndpi_struct,
		 NDPI_LOG_DEBUG, "exclude activesync\n");
