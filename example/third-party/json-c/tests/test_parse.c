...
static void test_incremental_parse()
{
	...
	for (ii = 0; incremental_steps[ii].string_to_parse != NULL; ii++) {
		...
		new_obj =
		    json_tokener_parse_ex(tok, step->string_to_parse, length);
		...
		if (step->expected_error != json_tokener_success) {
			if (new_obj != NULL)
				...
				printf
				    ("ERROR: wrong char_offset %d != expected %d\n",
				     tok->char_offset, expected_char_offset);
			else {
			...
			if (new_obj == NULL)
				printf
				    ("ERROR: expected valid object, instead: %s\n",
				     json_tokener_error_desc(jerr));
			else if (tok->char_offset != expected_char_offset)
				printf
				    ("ERROR: wrong char_offset %d != expected %d\n",
