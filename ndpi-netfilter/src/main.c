...
static struct osdpi_flow_node *ndpi_flow_search(struct rb_root *root,
						struct nf_conn *ct)
{
...
static struct osdpi_id_node *ndpi_id_search(struct rb_root *root,
					    union nf_inet_addr *ip)
{
	int res;
...
static struct osdpi_flow_node *ndpi_alloc_flow(struct nf_conn *ct)
{
	...
	flow->packets = 1;
	flow->ndpi_flow =
	    (struct ndpi_flow_struct *)((char *)&flow->ndpi_flow +
...
static struct osdpi_id_node *ndpi_alloc_id(union nf_inet_addr *ip)
{
	...
	if (id != NULL) {
		...
		id->ndpi_id =
		    (struct ndpi_id_struct *)((char *)&id->ndpi_id +
					      sizeof(id->ndpi_id));
		kref_init(&id->refcnt);
	...
	for (i = 1; i <= NDPI_LAST_IMPLEMENTED_PROTOCOL; i++) {
		if (NDPI_COMPARE_PROTOCOL_TO_BITMASK(info->flags, i) != 0) {
			...
			if (atomic_dec_and_test(&protocols_cnt[i - 1])) {
				NDPI_DEL_PROTOCOL_FROM_BITMASK
				    (protocols_bitmask, i);
...
static int ndpi_conntrack_event(struct notifier_block *this, unsigned long ev,
				void *data)
{
	...
	if (flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN
		...
		if (flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN)
			pr_info("PROTO %d detected, %s \n",
				...
				ndpi_get_proto_by_id(ndpi_struct,
						     flow->detected_protocol));
...
static int __init ndpi_mt_init(void)
{
	...
	osdpi_id_cache =
	    kmem_cache_create("xt_ndpi_ids",
			      sizeof(struct osdpi_id_node) + size_id_struct, 0,
			      0, NULL);
	if (!osdpi_id_cache) {
