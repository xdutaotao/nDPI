...
DEFINE_SPINLOCK (flow_lock);  // to keep osdpi_flow_root safe
DEFINE_SPINLOCK (id_lock);   // to keep osdpi_id_root safe
DEFINE_SPINLOCK (ipq_lock);  // to keep protocols_bitmask safe
...
static bool ndpi_free_flow (struct nf_conn *ct)
{
    struct osdpi_flow_node *flow;
	bool flag=false;
...
static void ndpi_free_id (union nf_inet_addr *ip)
{
    struct osdpi_id_node *id;
...
//release ct related flow and ct
static void ndpi_free_ct(struct nf_conn* ct)
{
...
// comment below conntrack notifier, clone another interface in kernel
#if 0
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
static int ndpi_conntrack_event (struct notifier_block *this, unsigned long ev, void *data)
{
...
static struct notifier_block osdpi_notifier = {
    .notifier_call = ndpi_conntrack_event,
};

#else
static int ndpi_conntrack_event (unsigned int events, struct nf_ct_event *item)
{
...
static struct nf_ct_event_notifier osdpi_notifier = {
...
/*
event hook when flow is deleted from conntrack
*/
static int ndpi_conntrack_delete_event (struct nf_conn *ct)
{
    ndpi_free_ct(ct);
    return 0;
}

static u32 ndpi_process_packet (struct nf_conn *ct, const uint64_t time, const struct iphdr *iph, uint16_t ipsize)
{
    ...
    if( (flow->packets > 2 && flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN) || flow->packets > 10)
    {
    ...
    if(dst == NULL)
    {
		//pr_info("new id, dst: %x \n",*ipdst);
        dst = ndpi_alloc_id (ipdst);
        if(dst == NULL)
        {
            pr_info ("xt_ndpi: dst == NULL\n");
            return proto;
        }
    }
    ...
    //put ct back into ctf, if meet below conditions
    // 1. if protocol is detected
    // 2. else if flow packages number > 10
    if( flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN || flow->packets > 10 )
    {
        if (flow->detected_protocol != NDPI_PROTOCOL_UNKNOWN) 
            pr_info ("PROTO %d detected, %s \n", flow->detected_protocol, ndpi_get_proto_by_id(ndpi_struct, flow->detected_protocol));
    
#ifdef HNDCTF
        // put the flow back into CTF
        ct->ctf_flags &= ~CTF_FLAGS_EXCLUDED;
#endif

        // cannot release the resources for temporary as flow is not terminated
        //ndpi_free_ct(ct);
    }else{
#ifdef HNDCTF
        //if we can catch something, keep flow out of CTF for a while
        ct->ctf_flags |= CTF_FLAGS_EXCLUDED;
#endif
        flow->packets ++;
    }
...
static bool ndpi_mt (const struct sk_buff *skb, const struct xt_match_param *par)
#else
...
#elif LINUX_VERSION_CODE < KERNEL_VERSION(2,6,35)
static bool ndpi_mt_check (const struct xt_mtchk_param *par)
{
    const struct xt_ndpi_mtinfo *info = par->matchinfo;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,28)
static void ndpi_mt_destroy (const struct xt_match *match, void *matchinfo)
{
    const struct xt_ndpi_mtinfo *info = matchinfo;
...
static int __init ndpi_mt_init (void)
{
    ...
    pr_info ("xt_ndpi 1.2 (nDPI wrapper module).\n");
    ...
    osdpi_flow_cache = kmem_cache_create ("xt_ndpi_flows",
                                          sizeof (struct osdpi_flow_node) + size_flow_struct, 0, 0, NULL);
    if(!osdpi_flow_cache)
    {
...
// here, we do not regiest notifier, for netlink reserved such notifier
// interface, so we just clone another new register into kernel
//nf_conntrack_delete_hook = 0;
#if 0
#if LINUX_VERSION_CODE < KERNEL_VERSION(3,2,0)
...
#endif
    nf_conntrack_delete_hook = ndpi_conntrack_delete_event;
